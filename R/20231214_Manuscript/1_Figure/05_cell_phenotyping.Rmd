...
spe <- readRDS(file.path(path,"results/Manuscript/1_Figure/spe_1px-curated.rds"))
...
#saveRDS(mat_red_clustering, file.path(path, "results/Manuscript/1_Figure/mat_red_clustering.rds"))

mat_red_clustering <- readRDS(file.path(path,"results/Manuscript/1_Figure/mat_red_clustering.rds"))
...
#saveRDS(clusters, file.path(path, "results/Manuscript/1_Figure/pg_clusters.rds"))

clusters <- readRDS(file.path(path, "results/Manuscript/1_Figure/pg_clusters.rds"))
...
for(i in 1: length(unique(clusters))){
  ...
  png(file=file.path(path, paste0("results/Manuscript/1_Figure/", t, "/", t, "_" , c , ".tiff")),width=18,height=8,units="in", res=1200)
...
Next we will visualize the clusters in a bubble plot. 
...
mean_morph <- aggregate(t(morph_mat_clipped), list(spe$pg_clusters), mean)
area_mean = mean_morph[,"area"]
solidity_mean = mean_morph[,"solidity"]
col_mean_area = colorRamp2(c(min(area_mean),min(area_mean)+(max(area_mean)-min(area_mean))/2, max(area_mean)), c("blue", "white", "red"))
col_mean_solidity = colorRamp2(c(min(solidity_mean),min(solidity_mean)+(max(solidity_mean)-min(solidity_mean))/2, max(solidity_mean)), c("blue", "white", "red"))
...
column_ha = HeatmapAnnotation(#pg_clusters = colnames(exp_mat),
                              ...
                              #area = area_mean,
                              #ncells_overall=anno_barplot(as.data.frame(table(spe$pg_clusters))[,"Freq"], add_numbers=T, height = unit(2.2, "cm")),
                              ...
                              #annotation_legend_param = list(pg_clusters = list(
...
#saveRDS(mat_red, file.path(path, "results/Manuscript/1_Figure/mat_red.rds"))

mat_red <- readRDS(file.path(path,"results/Manuscript/1_Figure/mat_red.rds"))
...
#write.csv(data_wide, file.path(path, "results/Manuscript/1_Figure/clusters-per-sample.csv"))
...
write.csv(data_wide, file.path(path, "results/Manuscript/1_Figure/samples-per-cluster.csv"))
...
write.csv(data_wide, file.path(path, "results/Manuscript/1_Figure/image-per-cluster.csv"))
...
#write.csv(data_wide, file.path(path, "/results/Manuscript/1_Figure/tissues-per-cluster.csv"))
...
#for(i in 1: 1){
  ...
  #           display = "single",
  #           save_plot = list(filename = file.path(path, "results/Manuscript/1_Figure/", tissue, paste0("all_", files[i], "f"))),
  #           legend = NULL)
  ...
  plotCells(masks,
            ...
            display="single",
            save_plot = list(filename = file.path(path, "results/Manuscript/1_Figure/", tissue, paste0(files[i], "f"))),
            legend=NULL
            )
...
```{r visualize-clusters-in-image, message=FALSE, fig.width=15}
library(dplyr)
annotation <- read.csv(file.path(path, "results/Manuscript/1_Figure", "annotation_v9.csv"), sep=";")
...
tiff(filename = file.path(path, paste0("results/Manuscript/2_Ext_Figure/R/GMM_heatmap.tiff")),
     width = 170, height = 150, units = "mm", res=600)
draw(ht)
dev.off()

setEPS()
postscript(file.path(path, "results/Manuscript/2_Ext_Figure/R/GMM_heatmap.eps"), height=5.9, width=6.7)
draw(ht)
dev.off()
```
...
tiff(filename = file.path(path, paste0("results/Manuscript/2_Ext_Figure/R/singleR_prediction-score.tiff")),
     width = 90, height = 55, units = "mm", res=600)
plotScoreHeatmap(predicted_labels, annotation_colors=list(Labels = col_clusters), annotation_legend=F, treeheight_row=0) #define the colors first 
dev.off()

setEPS()
postscript(file.path(path, "results/Manuscript/2_Ext_Figure/R/singleR_prediction-score.eps"), height=2.1, width=3.5)
plotScoreHeatmap(predicted_labels, annotation_colors=list(Labels = col_clusters), annotation_legend=F, treeheight_row=0) #define the colors first 
dev.off()
...
for(i in 1: 1){
  ...
  plotCells(masks,
            ...
            display="single",
            #save_plot = list(filename = file.path(path, "results/Manuscript/1_Figure", paste0("2", files[i], "f"))),
            #legend=NULL
            )
...
saveRDS(spe, file.path(path, "results/Manuscript/spe_clustered.rds"))
```
