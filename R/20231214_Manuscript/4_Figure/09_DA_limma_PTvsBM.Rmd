...
setEPS()
postscript(file.path(path, "results/Manuscript/4_Figure/R/barplot_PTvsBM_samples_paired.eps"), height=12, width=20)
p
dev.off()
```
...
heterogeneity <- as.data.frame(het)
heterogeneity[,"condition"] <- sapply(strsplit(rownames(heterogeneity), "_"), function(x) x[2])

test <- compare_means(het ~ condition, 
                        #comparisons = my_comparisons, 
                        p.adjust.method = "bonferroni", 
                        method='wilcox.test', 
                        data = heterogeneity)
setEPS()
postscript(file.path(path, "results/Manuscript/4_Figure/R/PTvsBM_het_paired.eps"), height=5, width=5)
ggboxplot(heterogeneity, x = "condition", y = "het",
                color = "condition", palette=c("slategray3", "slategray"),
                add.params = list(size = 3),
                add = "jitter")+ 
                stat_pvalue_manual(test, label = "p.adj", y.position = max(heterogeneity$het)) +
...
setEPS()
postscript(file.path(path, "results/Manuscript/4_Figure/R/annotationbar_PTvsBM_paired.eps"), height=30, width=30)
...
sce$sample_id <- factor(sce$sample_id, levels=clustered_samples)
 ...
 #absolute cellcount
setEPS()
postscript(file.path(path, paste0("results/Manuscript/4_Figure/R/PTvsBM_annotationbar_ncells.eps")), height=30, width=30)
...
setEPS()
postscript(file.path(path, "results/Manuscript/4_Figure/R/PTvsBM_tumor_boxplot_paired.eps"), height=26, width=20)
...
#Piechart
setEPS()
postscript(file.path(path, "results/Manuscript/4_Figure/R/PTvsBM_piechart.eps"), height=2, width=6.5)
