...
write.csv(cellular_communities, file.path(path, "results/Manuscript/5_Figure/R/cellular_communities.csv"))

#for DA between cellular communities
sce$sample_id <- factor(sce$sample_id, levels=clustered_samples)
sce$community_cluster <- sce$sample_id
sample_clusters <- setNames(paste0("c_", cellular_communities$community_cluster), rownames(cellular_communities))
sce$community_cluster <- as.factor(recode(sce$community_cluster, !!!sample_clusters))
metadata(sce)$experiment_info <- as.data.frame(colData(sce)) %>% group_by(sample_id,patient, mna, community_cluster, progress) %>%
    summarise(n_cells=n(),.groups = 'drop') %>%
    as.data.frame()

setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/dendogram_BM_DE_samples.eps"), height=12, width=30)
plot(hc)
dev.off()

setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/barplot_BM_DE_samples_celltype.eps"), height=12, width=20)
p
dev.off()
```
...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/barplot_BM_DE_samples_ME.eps"), height=12, width=20)
p_me
dev.off()
```
...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/BM_DE_annotation_bar.eps"), height=30, width=30)
...
#absolute cellcount
setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_annotationbar_ncells.eps")), height=30, width=30)
draw(anno_barplot(as.vector(unname(table(sce$sample_id))), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_annotationbar_ntumorcells.eps")), height=30, width=30)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_annotationbar_tumor_inf.eps")), height=30, width=30)
draw(anno_barplot(as.vector(tumor_infiltration), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_annotationbar_tumor_inf_norm.eps")), height=30, width=30)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_discrete.eps")), height=30, width=5)
do.call("grid.arrange", c(plot_list, ncol=1))
dev.off()
```
We will then plot continuous variables in boxplots. 

```{r summary_plots-con, message=FALSE, fig.height=35, fig.width=5}
library("ggpubr")
library(gtools)
...
#roundness
df <- aggregate(sce[,sce$metacluster=="granulocyte"]$solidity_clipped, list(sce[,sce$metacluster=="granulocyte"]$sample_id), mean)
rownames(df) <- df$Group.1
df <- df[clustered_samples,]
roundness <- as.vector(df$x)
...
cont_df <- data.frame(cbind(n_overall, n_tumor, perc_tumor, perc_tumor_norm, het_m, het_t, age, age_clipped, roundness, cellular_communities),row.names=clustered_samples)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_continuous_roundness.eps")), height=30, width=5)
do.call("grid.arrange", c(plot_list, ncol=1))
dev.off()
```
...
sce$sample_id <- factor(paste0(spe[keep_rows, keep_cols]$fm_id, "_", sce$tissue), levels=clustered_samples_short)
...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/BM_DE_cellular-communities_ME_boxplot_edgeR.eps"), height=26, width=20)
...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/BM_DE_cellular-communities_ME_barplots_voom.eps"), height=12, width=16)
grid.arrange(grobs = plot_list, ncol = 3)
   dev.off()

```
As a last step, we will plot all metacluster abundances ordered by initial clustering. 
```{r metaclusters, fig.width=20, fig.height=10}
sce$condition <- sce$tmp

setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/BM_DE_barplot_all_metaclusters.eps"), height=12, width=16)
plotAbundances(sce, k = "celltype", by = "sample", col_clust=F, k_pal=metadata(spe)$color_vectors$col_metacluster) 
dev.off()
```
