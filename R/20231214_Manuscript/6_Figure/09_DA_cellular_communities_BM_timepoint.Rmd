...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/barplot_BM_timepointamples.eps"), height=12, width=20)
p
dev.off()
```
...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/barplot_BM_timepointamples_ME.eps"), height=12, width=20)
p_me
dev.off()
```
...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/annoationbar_BM_timepoint.eps"), height=30, width=30)
...
#absolute cellcount
setEPS()
postscript(file.path(path, paste0("results/Manuscript/6_Figure/R/BM_timepoint_annotationbar_ncells.eps")), height=30, width=30)
draw(anno_barplot(as.vector(unname(table(sce$sample_id))), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/Manuscript/6_Figure/R/BM_timepoint_annotationbar_ntumorcells.eps")), height=30, width=30)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/6_Figure/R/BM_timepoint_annotationbar_tumor_inf.eps")), height=30, width=30)
draw(anno_barplot(as.vector(tumor_infiltration), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/Manuscript/6_Figure/R/BM_timepoint_annotationbar_tumor_inf_norm.eps")), height=30, width=30)
...
col_timepoint = setNames(c('#beaed4', '#386cb0', '#ffff99'), c("DE", "RE2", "REL")) 
...
cont_df <- data.frame(cbind(n_overall, n_tumor, perc_tumor, perc_tumor_norm, het_m, het_t, age, age_clipped, roundness, condition),row.names=clustered_samples)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/6_Figure/R/BM_timepoint_continuous_roundness.eps")), height=30, width=5)
do.call("grid.arrange", c(plot_list, ncol=1))
dev.off()
...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/barplot_BM_timepoint_metaclusters_samples.eps"), height=12, width=20)
p_all
dev.off()
```
```