...
spe_tumor <- readRDS(file.path(path,"results/public_datasets/Jansky/results/spe_predicted_tumor_clusters.rds"))
spe_tumor$predicted_clusters <- spe_tumor$predicted_tumor_clusters
spe_tumor$predicted_tumor_clusters <- NULL

spe_non_tumor <- readRDS(file.path(path,"results/public_datasets/Lee/results/spe_immune_clusters.rds"))
...
saveRDS(spe, file.path(path, "results/public_datasets/Jansky_Lee/spe_predicted-clusters_Jansky-Lee_2.RDS"))
```
...
```{r barplot-cluster-per-sample, fig.height=5, fig.width=4}
 # library
library(ggplot2)
library(tidyverse)
...
spe_reduced <- spe[,spe$control=="no" & spe$metacluster=="tumor" & spe$tissue=="BM" & spe$timepoint=="DE"]
...
ggplot(data_long, aes(fill=celltype, y=count, x=sample)) + 
  ggtitle(paste0("BM_celltype per ", groupname)) +
