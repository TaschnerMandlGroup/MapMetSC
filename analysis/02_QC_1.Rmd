...
params: 
  input: "/mnt/Multimodal_Imaging_Daria/Publication/test_data"
  output: "/mnt/Multimodal_Imaging_Daria/Publication/test_data/data"
---
...
```{r read-steinbock, cache=TRUE, dependson=character(0)}
path <- params$output
...
n <- length(non_zeros[non_zeros==FALSE])
```
We excluded `r n` cells in this first quality control step.
...
col_sample = setNames(sample(col_vector_433, length(unique(colnames(mat)))), unique(colnames(mat)))

set.seed(221228)
rand_cells <- sample(seq_len(ncol(mat)), 2000)

column_ha = HeatmapAnnotation(sample = colnames(mat[,rand_cells]), 
...
c = 11
celltype <- spe[,spe$pg_clusters_lostcells == c & spe$sample_id == max_sample]
...
mask_plot <- plotCells(masks,
                       ...
                       img_id = "sample_id",
                       colour_by = "pg_clusters_lostcells",
                       colour = list(celltype = c(celltype = "red")),
                       missing_colour = "white",
                       #save_plot = list(filename = file.path(params$output, paste0(max_sample, "_", c, ".png"))),
                       legend=NULL,
                       return_plot = TRUE)
