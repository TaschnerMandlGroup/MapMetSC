...
if (compartment=="tumor"){
  ...
  BM_patients_2 <- unique(spe[,spe$tissue=="BM" & spe$control=="no" & spe$metacluster == "tumor" & spe$timepoint == timepoint]$study_id )
  paired_patients <- intersect(BM_patients_1, BM_patients_2)
  keep_cols <- spe$celltype != "other" & spe$tissue=="BM" & spe$metacluster=="tumor" & spe$control=="no" & (spe$timepoint=="DE" | spe$timepoint==timepoint) & spe$study_id%in%paired_patients 
}else{
  BM_patients_1 <- unique(spe[,spe$tissue=="BM" & spe$control=="no" & spe$timepoint == "DE"]$study_id )
  BM_patients_2 <- unique(spe[,spe$tissue=="BM" & spe$control=="no" & spe$timepoint == timepoint]$study_id )
  paired_patients <- intersect(BM_patients_1, BM_patients_2)
  keep_cols <- spe$celltype != "other" & spe$tissue=="BM" & spe$metacluster!="tumor" & spe$metacluster!="mesenchymal" & spe$control=="no" & (spe$timepoint=="DE" | spe$timepoint==timepoint) & spe$study_id%in%paired_patients 
}
