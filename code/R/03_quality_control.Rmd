...
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20221010_Steinbock_Mesmer_IFbased"
...
metadata(spe)$color_vectors$sample <- setNames(sample(col_vector_433, n), 
                unique(spe$sample))
```
...
#spe_bm <- spe_all[,spe_all$tissue == 'BM']
#spe_pt <- spe_all[,spe_all$tissue == 'PT']
#spe <- spe_pt

cur_cells <- sample(seq_len(ncol(spe)), 20000)

#pdf(file.path(path,"data/sc_heatmap_combined.pdf"), height=7, width=12)
...
set.seed(220224)
mat <- apply(assay(spe, "compexprs"), 1, function(x){
...
#pdf(file.path(path,"data/BM-PT_multi-ridgeplot.pdf"), height=35, width=7)
...
```{r ridges, message=FALSE, fig.width=14, fig.height=35}
pdf(file.path(path,"data/PT/outlier_samples_multi-ridgeplot_tissue.pdf"), height=35, width=14)

#selected_samples <- c("20211227_09-0920_BM", "20220222_16-4503_BM", "20220223_06-2373_BM", "20220221_02-2370_BM", "20220303_18-4528_BM", "20221019_19-5754_BM", "20220421_21-0850_BM")
selected_samples <- c("20220804_18-1061_TU", "20220816_11-1208_TU", "20220907_05-3231_TU", "20220906_03-0170_TU", "20220809_16-1927_TU")

multi_dittoPlot(spe[,spe$sample%in%selected_samples], vars = rownames(spe)[rowData(spe)$use_channel],
               group.by = "sample", plots = c("ridgeplot"), 
               assay = "exprs"#,
               #color.panel = metadata(spe)$color_vectors$year
               )
dev.off()
...
set.seed(220225)
spe <- runUMAP(spe, subset_row = rowData(spe)$use_channel, exprs_values = "compexprs", name="UMAP_asinh_comp", BPPARAM = MulticoreParam()) 
spe <- runUMAP(spe, subset_row = rowData(spe)$use_channel, exprs_values = "exprs", name="UMAP_asinh", BPPARAM = MulticoreParam()) 
...
p1 <- dittoDimPlot(spe, var = "sample", reduction.use = "UMAP_asinh_comp", size = 0.2, legend.show = FALSE) + 
    #scale_color_manual(values = metadata(spe)$color_vectors$year) +
    ggtitle("year on UMAP")
p2 <- dittoDimPlot(spe, var = "sample", reduction.use = "TSNE", size = 0.2, legend.size=2) + 
    #scale_color_manual(values = metadata(spe)$color_vectors$year) +
...
```{r visualizing-umap-by-samples, message=FALSE, fig.height=20}

spe <- spe[, sample(1:length(colnames(spe)))]

p3 <- dittoDimPlot(spe, var = "staining_date", reduction.use = "UMAP", size = 0.2, legend.show = T, opacity=1, order="randomize") +
    #scale_color_manual(values = metadata(spe)$color_vectors$sample) +
    #scale_color_manual(values = col_vector_433) +
    ggtitle("staining_date")
...
p3
```
We will now look at a multi-umap plot, visualizing one sample per plot while graying out the others. 

```{r visualizing-multiumap-by-samples, message=FALSE, fig.width=7, fig.height=20}
...
#path <- paste("/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20221220_Cellpose_IFbased/data/", tissue, sep="")
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20221220_Cellpose_IFbased/data/"

samples <- unique(spe[[var]])
...
for(i in 1: length(samples)){
  spe$sample_binary <- spe[[var]] == samples[i] 
  
  plot_list[[i]] <- dittoDimPlot(spe, var = "sample_binary", reduction.use = "UMAP_asinh_comp", size = 0.04, legend.show = F, opacity=1, order = "increasing") +
    scale_color_manual(values = bin_col) + 
    ggtitle(samples[i]) +
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(path, paste(tissue, "_UMAP_single", var, "_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
...
Let's now look at density plots for better visualization.
...
spe$old_new <- NA
spe[,as.integer(spe$year) <= 11]$old_new <- "old" 
spe[,as.integer(spe$year) > 11]$old_new <- "new" 
...
for(i in 1: length(samples)){
  ...
  background <- as.data.frame(reducedDim(spe[,spe$sample_binary == FALSE], "UMAP_asinh_comp"))
  foreground <- as.data.frame(reducedDim(spe[,spe$sample_binary == TRUE], "UMAP_asinh_comp"))
...
save_tiff <- function(sublist, j){
  plot <- plot_grid(plotlist = sublist, nrow=11, ncol=3) 
  ggsave(filename=file.path(path, paste(tissue, "_density_stat2d-filled_single_", var, "_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
