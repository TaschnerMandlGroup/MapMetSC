...
spe_all <- readRDS(file.path(path,"data/BM_spe.rds"))
...
pdf(file.path(path,"data/multi-ridgeplot_reduced.pdf"), height=35, width=7)
...
```{r visualizing-umap-by-samples, message=FALSE, fig.height=13}

p3 <- dittoDimPlot(spe, var = "stage", reduction.use = "UMAP", size = 0.2, legend.show = T, opacity=1) +
...
pdf(file.path(path,"data/multi-dimplot_reduced.pdf"), height=25, width=7)
multi_dittoDimPlot(
  spe,
  vars = rownames(spe)[rowData(spe)$use_channel],
