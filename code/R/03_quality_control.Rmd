...
p3 <- dittoDimPlot(spe, var = "timepoint", reduction.use = "UMAP", size = 0.2, legend.show = T, opacity=1) +
...
spe <- spe_bm
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20221102_Steinbock_Mesmer_IFbased/data/BM"
...
for(i in 1: length(samples)){
  spe$sample_binary <- spe$sample == samples[i] 
  
  plot_list[[i]] <- dittoDimPlot(spe, var = "sample_binary", reduction.use = "UMAP", size = 0.04, legend.show = F, opacity=0.2) +
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(path, paste("PT_UMAP_singlesample_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
