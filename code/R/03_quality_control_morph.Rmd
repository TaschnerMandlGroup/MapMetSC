...
#saveRDS(mat_red, file.path(path, "data/mat_red_morph.rds"))

mat_red <- readRDS(file.path(path,"data/mat_red_morph.rds"))
```
...
mat_red <- cbind(mat_red, t(features_mat)) # change this later!!!!!
...
ggsave(filename=file.path(out_path, "BM_UMAP_single_marker_morph_clipped_0-1.tiff"), width = 7, height = 30, units = "in", device='tiff', dpi=500)
...
```{r visualizing-umap-by-marker, message=F}

marker <- "Myeloperoxidase MPO"
mat_red_df <- as.data.frame(mat_red)
mat_red_df["marker"] <- features_mat[marker,]
mat_red_df_rand <- mat_red_df[sample(1:nrow(mat_red_df)),]
#mat_red_df_rand <- mat_red_df[order(features_mat[marker,], decreasing=F),]
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(out_path, paste("BM_morph_UMAP_single_sample_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
