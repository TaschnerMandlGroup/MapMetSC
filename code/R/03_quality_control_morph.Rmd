...
spe <- readRDS(file.path(path,"data/spe_1px.rds"))
...
intensity_mat_hq <- intensity_mat[!grepl("CD274|IDO|DAPI|2IF_GD2|3IF_CD56|DNA1|H4K12Ac|H3K9Ac|CXCR2|PNMT", rownames(intensity_mat)),]
intensity_mat_lq <- intensity_mat[grepl("CD274|IDO|DAPI|2IF_GD2|3IF_CD56|DNA1|H4K12Ac|H3K9Ac|CXCR2|PNMT", rownames(intensity_mat)),]
...
morph_mat_scaled_wo_zeros <- morph_mat_scaled[, non_zeros]


features_mat <- rbind(intensity_mat_hq_wo_zeros, morph_mat_scaled_wo_zeros)
...
column_ha = HeatmapAnnotation(cluster = unique(clusters), 
                              DAPI = dapi,
                              Ir= ir,
                              H3 = h3,
                              H4 = h4,
                              ncells=anno_barplot(as.data.frame(table(clusters))[,"Freq"], height = unit(3, "cm")),
                              col=list(cluster=col_clusters, DAPI=col_marker, Ir=col_marker, H3=col_marker, H4=col_marker), 
                              show_legend=T)
...
```
We can see that cluster 42 are the sticky cells, which are positive for everything. Cluster 29 and 1 are most probably cells that swam off between IF and IMC and are hence negative for everything except for DAPI. We will exclude cluster 1 and 29 for dimensionality reduction as UMAP performs better when the data is not as sparse.
...
mat <- mat[,!clusters %in% c("1", "29")]

mat_red <- calculateUMAP(mat, BPPARAM = MulticoreParam()) 
rownames(mat_red) <- colnames(mat)

saveRDS(mat_red, file.path(path, "data/mat_red_int.rds"))


#mat_red <- readRDS(file.path(path,"data/mat_red_int.rds"))
```
Next we will visualize the PG clusters in the UMAP space.

```{r visualizing-umap-by-phenograph-cluster, message=F, fig.width=20}
...
Now, we will plot specific markers in the dim-red space. 

```{r visualizing-umap-by-marker, message=F, fig.width=20}

marker <- "155Gd-GD2_Gd155_mean"
mat_red_df <- as.data.frame(mat_red)
mat_red_df["marker"] <- mat[marker,]
#mat_red_df_rand <- mat_red_df[sample(1:nrow(mat_red_df)),]
mat_red_df_rand <- mat_red_df[order(mat[marker,], decreasing=F),]
...
ggsave(filename=file.path(out_path, "BM_UMAP_single_marker_int.tiff"), width = 7, height = 30, units = "in", device='tiff', dpi=500)
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(path, paste("/data/BM-int_UMAP_single_sample_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
