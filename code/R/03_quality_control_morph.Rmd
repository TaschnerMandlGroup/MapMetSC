...
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20230112_DIMR_Ilastik/sc_data"
...
intensity_mat_scaled <- intensity_mat_scaled[!grepl("CD274|IDO|DAPI|2IF_GD2|3IF_CD56|DNA1|H4K12Ac|H3K9Ac", rownames(intensity_mat_scaled)),]
...
mat <- intensity_mat_scaled
mat_wo_zeros <- mat[,as.logical(colSums(mat)!=0)]

mat_wo_zeros_red <- calculateUMAP(mat_wo_zeros, BPPARAM = MulticoreParam()) 
rownames(mat_wo_zeros_red) <- colnames(mat_wo_zeros)

saveRDS(mat_wo_zeros_red, file.path(path, "data/mat_wo_zeros_red_int.rds"))
...
mat <- t(intensity_mat_scaled)
mat_wo_zeros <- mat[as.logical(rowSums(mat)!=0),]
out <- FastPG::fastCluster(data=mat_wo_zeros, k=45, num_threads=n_threads)
...
set.seed(221228)
rand_cells <- sample(seq_len(ncol(mat_wo_zeros)), 2000)

column_ha = HeatmapAnnotation(cluster = clusters[rand_cells], col=list(cluster=col_clusters), show_legend=F)
Heatmap(mat_wo_zeros[,rand_cells], 
        column_title = "Single-cell heatmap with phenograph clusters",
        top_annotation = column_ha, 
        cluster_columns=F,
