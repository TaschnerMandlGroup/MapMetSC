---
title: "Clustering on full datset"
...
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20221102_Steinbock_Mesmer_IFbased/data/PT"
spe_all <- readRDS(file.path(path,"spe.rds"))
...
spe$pg_clusters_fastMNN <- clusters
```

```{r rphenograph-clustering-1, message=FALSE, fig.width=20}
dittoDimPlot(spe, var = "pg_clusters_fastMNN", 
             reduction.use = "UMAP_mnnCorrected", size = 0.2,
             do.label = TRUE) +
    ggtitle("Phenograph clusters expression on UMAP_mnnCorrected")
```

```{r phenograph-heatmap-1, fig.width=20}

tissue_col <- setNames(dittoColors(1)[1:length(unique(spe$tissue))],
                unique(spe$tissue))

clusters_col <- setNames(rep(dittoColors(1),times=5)[1:length(unique(spe$pg_clusters_seurat))],
                sort(unique(spe$pg_clusters_seurat)))

dittoHeatmap(spe[,cur_cells], 
             genes = rownames(spe)[rowData(spe)$use_channel],
             assay = "exprs", scale = "none",
             cluster_cols = FALSE,
             order.by = c("pg_clusters_seurat"),
             heatmap.colors = viridis(100), 
             annot.by = c("pg_clusters_seurat"),
             annotation_legend = FALSE,
             fontsize=10,
             annotation_colors = list(clusters=clusters_col))
```

```{r rphenograph-barplot-clusters-per-sample, message=FALSE, fig.width=40}
# by sample_id

clusters_col <- setNames(rep(dittoColors(1),times=5)[1:length(unique(spe$pg_clusters_seurat))],
                sort(unique(spe$pg_clusters_seurat)))

dittoBarPlot(spe, var = spe$pg_clusters_seurat, group.by = "sample", color.panel=clusters_col) #+
    #scale_fill_manual(values = metadata(spe)$color_vectors$celltype)
```

```{r rphenograph-barplot-samples-per-cluster, message=FALSE, fig.width=40}
# by clusters
dittoBarPlot(spe, var = spe$sample, group.by = "pg_clusters_fastMNN") #+
    #scale_fill_manual(values = metadata(spe)$color_vectors$celltype)
```
...
```{r flowSOM-clusteirng-1, message=FALSE, fig.width=20}
...
```{r flowSOM-2-visualize-clusters, fig.height=8}
...
dittoDimPlot(spe, var = "som_clusters_corrected", 
             reduction.use = "UMAP_mnnCorrected", size = 0.2,
             do.label = TRUE) +
    ggtitle("SOM clusters expression on UMAP-mnnCorrected")
```

```{r flowSOM-2-heatmap, fig.height=8}
dittoHeatmap(spe[,cur_cells], 
             genes = rownames(spe)[rowData(spe)$use_channel],
             assay = "exprs", scale = "none",
             heatmap.colors = viridis(100), 
