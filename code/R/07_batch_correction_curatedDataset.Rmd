---
title: "Batch correction on curated dataset (without sticky cells)"
...
spe <- readRDS(file.path(path,"data/spe_curated_pg.rds")) # change this if you want to use the full dataset (with sticky cells)
```
...
We will now again visualize the cells in low dimensions after UMAP embedding for corrected and un-corrected data.

```{r dimred-batch-uncorrected, message=FALSE, eval=FALSE}
set.seed(220225)
...
plot_list <- multi_dittoDimPlot(spe, var = rownames(spe)[rowData(spe)$use_channel], reduction.use = "UMAP", 
                   assay = "exprs", size = 0.2, list.out = TRUE,  min.color="gray90", max.color="red", opacity=0.5) 
...
plot_list <- multi_dittoDimPlot(spe, var = rownames(spe)[rowData(spe)$use_channel], reduction.use = "UMAP_harmony", 
                   assay = "exprs", size = 0.2, ncol=3, nrow=13, list.out = TRUE,  min.color="gray90", max.color="red", opacity=0.5) 
plot_list <- lapply(plot_list, function(x) x)
plot_grid(plotlist = plot_list, nrow=13, ncol=3)
```
