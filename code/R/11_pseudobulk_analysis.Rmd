...
spe_bm <- readRDS(file.path(path,"data/BM/spe_bm.rds")) # change this if you want to use the full dataset (with sticky cells)
```
...
active_var <- colnames(exprs_df[,!colnames(exprs_df) %in% c("136Ba", "131Xe", "134Xe", "Histone H3K9Ac", "H4K12Ac", "Iridium191", "Iridium193", "DAPI", "CD56-IF", "GD2-IF")])
...
fviz_cos2(res.pca, choice = "ind", axes = 2, top = 10)
...
pseudo_mat_quant <- cbind(exprs_mat, "imc_score"=meta_mat[,"imc_score"])
