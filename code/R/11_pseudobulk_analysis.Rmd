...
spe <- readRDS(file.path(path,"data/BM/spe_bm.rds")) # change this if you want to use the full dataset (with sticky cells)
```
...
exprs_df <- exprs_df[,!colnames(exprs_df) %in% c("136Ba", "131Xe", "134Xe")]
...
pseudo_mat <- cbind(exprs_mat, meta_mat)
```
...
pseudo_mat.active <- pseudo_mat[,colnames(exprs_df)]
...
```{r PCA, echo=FALSE}
library("FactoMineR")
res.pca <- PCA(pseudo_mat.active, graph = FALSE)
...
eig.val
```
Eigenvalues can be used to determine the number of principal components to retain after PCA. In our analysis, the first five principal components explain 72% of the variation. This is an acceptably large percentage.
...
```{r graph-groups, echo=FALSE, fig.width=20}
...
fviz_pca_ind(res.pca,
             ...
             col.ind = pseudo_mat_df$year, # color by groups
             palette = col_vector_74,
...
pseudo_mat <- pseudo_mat[,!colnames(pseudo_mat) %in% c("sample", "study_id", "timepoint", "control", "year", "staining_date", "IMC_date", "stainer")]
pseudo_mat <- data.frame(apply(pseudo_mat, 2, function(x) as.numeric(as.character(x))))
res.pca <- PCA(pseudo_mat, quanti.sup=45, graph=FALSE)
...
summary <- facto_summarize(res.pca, "ind", axes=1:5)

contrib_meta <- cbind(summary, meta_mat)

stainer_contrib <- aggregate(contrib~stainer,contrib_meta,mean)
