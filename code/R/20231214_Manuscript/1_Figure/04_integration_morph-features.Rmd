...
spe <- readRDS(file.path(path,"results/Manuscript/1_Figure/spe_0px_wo-lostcells.rds"))
...
spe_bm$outlier <- "no"
spe_bm[,spe_bm$area <= 250]$outlier <- "yes"

sum(spe_pt$area <= 100)

spe_pt$outlier <- "no"
spe_pt[,spe_pt$area <= 100]$outlier <- "yes"
```
We have 7923 outliers among the BM samples and 922 among the PT samples. 
...
data <- data.frame(sample=spe_bm$sample_id, cluster = spe_bm$outlier)
data$freq <- 1
...
The sample with the most outlier cells is 20220806_20220314_17-4510_BM_004. We will pseudo-color the masks of outlier cells in the sample with the highest presence. 
...
saveRDS(spe, file.path(path, "results/Manuscript/1_Figure/spe_0px-curated.rds"))
...
We are excluding 8847 segmentation artifacts based on morphology. 
...
write.csv(morph_pseudobulk, file.path(path, "results/Manuscript/1_Figure/morph_pseudobulk.csv"))
...
##Save data
```{r pseudobulk, message=FALSE}
spe_1px <- readRDS(file.path(path, "results/Manuscript/1_Figure/spe_1px_refined.rds"))
spe_1px <- spe_1px[,colnames(spe)]

saveRDS(spe_1px, file.path(path, "results/Manuscript/1_Figure/spe_1px-curated.rds"))
```
