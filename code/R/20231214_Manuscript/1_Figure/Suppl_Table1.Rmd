...
#Anonymize sample ID
codes <- setNames(paste0("Sample_", sprintf("%03d", 1: length(unique(meta_all$Sample)))), unique(meta_all$Sample))
...
write.csv(meta_all, file.path(path, "results/Manuscript/1_Figure/R/Suppl_Table1_anonymization-codes.csv"))
...
write.csv(meta_anon, file.path(path, "results/Manuscript/1_Figure/R/metadata_complete_anon.csv"))
...
meta_patient_clinical <- meta_all[meta_all$control=="no",c("Patient_ID", "stage4", "rez_dat", "prog_dat", "event_dat", "tod_dat1", "efs", "tod", "efs_dur", "tod_dur", "age_at_diag", "therapy")]
...
write.xlsx(list_of_datasets, file = file.path(path, "results/Manuscript/1_Figure/R/Suppl_Table1.xlsx"))
```
