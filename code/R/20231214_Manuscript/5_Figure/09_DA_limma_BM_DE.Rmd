...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/barplot_BM_DE_NoProgvsProg_samples.eps"), height=12, width=20)
p
dev.off()
```
...
setEPS()
postscript(file.path(path, "results/Manuscript/3_Figure/R/annotationbar_BM_crtlvsDE.eps"), height=30, width=30)
...
condition <- meta_all$efs

#retrieve overall cell number
temp <- as.data.frame(table(spe[, paste0(spe$fm_id, "_", spe$tissue)%in%clustered_samples]$sample))
rownames(temp) <- sapply(strsplit(as.character(temp$Var1), "_"), function(x) paste(x[3], x[4], sep = "_"))
temp <- temp[clustered_samples, ]
...
cont_df <- data.frame(cbind(perc_tumor, het, age, age_clipped, roundness, condition),row.names=clustered_samples)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_NoProgvsProg_continuous_roundness.eps")), height=30, width=5)
...
for(i in 1:length(unique(p$data$cluster_id))){
  ...
  plot_list_fc[[i]] <- ggboxplot(proportions, x = "condition", y = "Freq", order=c("NoProg", "Prog"),
...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/BM_DE_NoProgvsProg_ME_MNA_boxplot_voom.eps"), height=26, width=20)
...
setEPS()
postscript(file.path(path, "results/Manuscript/5_Figure/R/BM_DE_NoProgProg_ME_MNA_barplots_voom.eps"), height=12, width=16)
  p
dev.off()
