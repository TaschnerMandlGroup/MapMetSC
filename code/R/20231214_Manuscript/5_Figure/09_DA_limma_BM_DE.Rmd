...
keep_cols <- spe$celltype != "other" & spe$timepoint=="DE" & spe$metacluster!="tumor" & spe$metacluster!="mesenchymal"  & spe$tissue=="BM" & spe$MYCN_amp==1 
...
cont_df <- data.frame(cbind(perc_tumor, het, age, age_clipped, condition),row.names=clustered_samples)
...
setEPS()
postscript(file.path(path, paste0("results/Manuscript/5_Figure/R/BM_DE_NoProgvsProg_continuous.eps")), height=30, width=5)
