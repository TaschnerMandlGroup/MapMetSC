...
keep_cols <- spe$celltype != "other" & spe$tissue=="BM" & spe$metacluster=="tumor" & spe$control=="no" #spe$study_id%in%paired_patients & (spe$timepoint=="DE" | spe$timepoint=="RE1" | spe$timepoint=="RE2")#& spe$MYCN_amp==1 & spe$progression==1
...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/barplot_BM_DEvsRE1vsRE2_samples_paired.eps"), height=12, width=20)
p
dev.off()
```
...
for(i in 1:length(unique(p$data$cluster_id))){
  ...
  plot_list_fc[[i]] <- ggboxplot(proportions, x = "condition", y = "Freq", order=c("DE", "RE1", "RE2"),
...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/BM_timepoint_tumor_boxplot_DERE1RE2_paired_edgeR.eps"), height=26, width=20)
...
for(i in 1:length(unique(p$data$cluster_id))){
  ...
  plot_list_fc[[i]] <- ggboxplot(proportions, x = "condition", y = "Freq", order=c("DE", "REL"),
...
setEPS()
postscript(file.path(path, "results/Manuscript/6_Figure/R/BM_timepoint_tumor_boxplot_DEvsREL_paired_edgeR.eps"), height=26, width=20)
grid.arrange(grobs = plot_list_fc, ncol = 6)
   dev.off()
```
