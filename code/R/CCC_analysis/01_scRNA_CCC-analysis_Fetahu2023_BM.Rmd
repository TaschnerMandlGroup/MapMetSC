...
Same as in the publication Fetahu et al., 2023
...
DEG_tumor <- DoHeatmap(fetahu.NB, features = top_10$feature, assay = "RNA")
...
barplot1 <- ggplot(pt, aes(x = Var2, y = Freq, fill = factor(Var1))) +
  theme_bw(base_size = 15) +
  geom_col(position = "fill", width = 0.5) +
...
# Cell type annotation predicted from Original data
Idents(rna) <- rna@meta.data$predicted_celltype
...
barplot2 <- ggplot(pt, aes(x = Var2, y = Freq, fill = factor(Var1))) +
  theme_bw(base_size = 15) +
  geom_col(position = "fill", width = 0.5) +
