...
groups_of_interest <- c("II","III","IV")

rna <- subset(rna, idents = "I", invert = TRUE) # remove controls (group I)
...
rna@meta.data <- rna@meta.data %>% 
  mutate(new_groups = ifelse((rna@meta.data$group == MYCNA_patients ),  
                             "MYCNA","non_MYCNA"))
...
combined_umap <- DimPlot(rna, cols = COLOR_CODE_RNA_v3, seed = 28, shuffle = TRUE) + 
  ggtitle(label = "Neuroblastoma") + NoLegend() +  
  theme(text = element_text(size = 20), axis.text.x = element_text(size = 18),
        axis.text.y = element_text(size = 18)) # n = 26726 cells
mycn_umap <- DimPlot(MYCNA, cols = COLOR_CODE_RNA_v3, seed = 28, shuffle = TRUE) + 
  ggtitle(label = "MYCN amplified") + NoLegend() +  
  theme(text = element_text(size = 20), axis.text.x = element_text(size = 18),
        axis.text.y = element_text(size = 18)) # n = 10155 cells
non_mycn_umap <- DimPlot(nonMYCNA, cols = COLOR_CODE_RNA_v3, seed = 28, shuffle = TRUE) + 
  ggtitle(label = "MYCN wildtype") +  
  theme(text = element_text(size = 20), axis.text.x = element_text(size = 18),
...
# save plots
pdf(paste0(params$out,"Figures/fetahu2023_UMAPs_MYCNA_nonMYNCA.pdf"),
    height = 7, width = 21)
combined_umap + mycn_umap + non_mycn_umap  
dev.off()

png(filename = paste0(params$out,"Figures/fetahu2023_UMAPs_MYCNA_nonMYNCA.png"),
    width = 2100, height = 700, units = "px", pointsize = 1, res = 150)
combined_umap + mycn_umap + non_mycn_umap  
dev.off()

tiff(filename = paste0(params$out,"Figures/fetahu2023_UMAPs_MYCNA_nonMYNCA.tiff"),
...
expressed_genes_receiver <- get_expressed_genes(receiver, rna, pct = 0.05)
...
1. non-MYCNA (ATRXm and sporadic) vs. MYCNA 
2. MYCNA vs. non-MYCNA (ATRXm and sporadic)
