...
rna_mat_scaled <- rna_mat_scaled %>% drop_na()
...
saveRDS(fetahu.NB, 
        file = paste0(params$out,"Rds/fetahu2023.scRNA.tumor.clusters.Rds"))
...
DoHeatmap(fetahu.NB, feature = marker_results_top5$feature)
...
# Subset to TME only
tumor_cells <- "NB (8)"
fetahu.NB <- subset(rna, idents = tumor_cells, invert = TRUE)
...
# plot the distribution of neighbourhood sizes (i.e. how many cells form each neighbourhood) 
# to evaluate whether the value of k used for graph building was appropriate
plotNhoodSizeHist(rna_milo)

# Counting cells in neighbourhoods
rna_milo <- countCells(rna_milo, meta.data = as.data.frame(colData(rna_milo)), sample="sample")
...
umap_pl <- plotReducedDim(rna_milo, dimred = "umap", colour_by="group", 
                          text_by = "original_celltype", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")

# Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(rna_milo, da_results, layout="umap",alpha=0.1) 
