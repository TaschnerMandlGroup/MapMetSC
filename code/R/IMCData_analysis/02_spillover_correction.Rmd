...
```
We can see that when we use binning, so summing up consecutive pixels, then the result gets less accurate, e.g. we se no spillover from 167 (H4K12Ac) to 168 (GATA3), which we know we have based on the IMC images of the compensation experiment. The computeSpillMat excludes all interactions that are considered chemically and physically non-sensible, which is why we do not see the spillover from Pt196 to Lu175 anymore. Based on detailed examination, we know that there was an error during the spillover experiment. We therefore decide to use the spillover matrix without binning and default interactions for the compensation. 
...
library(parallel)
images <- getChannels(images, 1:44) #excluding IF channels
images_short <- images[sample(length(images), 20)]
saveRDS(images, file.path(path,"data/images.rds"))
rm(images)
gc()
images_comp <- compImage(images_short, adapted_sm) 
...
For convenience, we will re-set the `channelNames` to their biological targets:
