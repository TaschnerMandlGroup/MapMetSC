...
Now we will highlight expression similarities between samples. 

```{r celltype pbmds, message=FALSE, fig.width=20}
# MDS pseudobulk by cell type
...
condition = "staining_date"

spe_cat <- spe
col_condition = setNames(sample(col_vector_74, length(unique(spe_cat[[condition]]))), unique(spe_cat[[condition]]))

spe_cat$sample_id <- factor(spe_cat$sample_id)
spe_cat$cluster_id <- factor(spe_cat$stainer)
spe_cat$condition <- factor(spe_cat[[condition]])

pbMDS(spe_cat, by = "sample_id", 
       features = rownames(spe)[rowData(spe_cat)$use_channel],
       color_by="condition") + 
  scale_color_manual(values = col_condition)

#clrDR(spe_cat, by = "sample_id")
```
