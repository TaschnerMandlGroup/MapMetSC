...
meta_all = read.csv(file.path(meta_path, "20230314_metadata_complete_comma.csv"))
meta_all <- meta_all[order(meta_all$Study_ID),] #order by study-id

exclude <- replace(x = meta_all$imc_exclude, list =  !meta_all$exclude %in% c('maybe'), values =  'no')
...
meta_positive <-meta_positive[!meta_positive$SCA_exclude == 'yes',]
