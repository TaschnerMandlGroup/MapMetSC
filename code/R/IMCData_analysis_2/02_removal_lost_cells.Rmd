...
spe <- readRDS(file.path(path,"results/spe_no-dil_non-refined.rds"))
...
```
We are excluding 6031 due to zero intensity. 
...
saveRDS(clusters, file.path(path, "results/clusters-int-no-dil-mean80_identify-lost-cells.rds"))
...
```
We can see that cluster 43 are most probably the sticky cells, which are positive for everything. Cluster 9 are most probably cells that swam off between IF and IMC and are hence negative for everything except for DAPI. We will check which sample has the highest presence of this cluster.
...
```
Sample 20220316_04-0169_BM_N_ROI_001 has the highest presence of cluster 22. Let's now color its mask by the clusters. Therefore we will just copy the img and mask of that sample into the "img" and "masks" folder as reading in all images takes too long. 
...
By overlaying the DAPI image, the Iridium image and the binary mask, we see that cluster 22 is the cluster of lost cells (n=28177 -1.38% of all (n=2034035) cells - 648images, on average 3139 cells per image  i.e. 43 cells per image). We will exclude those cells.

```{r exclude-lost-cells-in-1-px}

spe <- spe[, spe$pg_clusters!="9"]
saveRDS(spe, file.path(path, "results/spe_no-dil_non-refined_curated.rds"))
