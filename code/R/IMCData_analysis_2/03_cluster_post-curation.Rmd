...
```
We can see that cluster 42 are the sticky cells, which are positive for everything. We will exclude this cluster and re-generate the heatmap.
...
#Remove sticky cells
mat_wo_sticky <- mat[,clusters !=42]
mat_lq_wo_sticky <- mat_lq[,clusters !=42]
clusters_wo_sticky <-clusters[clusters !=42]
clusters_wo_sticky <-droplevels(clusters_wo_sticky)

mean <- aggregate(t(mat_wo_sticky), list(clusters_wo_sticky), mean)
mean_lq <- aggregate(t(mat_lq_wo_sticky), list(clusters_wo_sticky), mean)
...
column_ha = HeatmapAnnotation(cluster = mean[,"Group.1"], 
                              ...
                              col=list(cluster=col_clusters[-43] 
                                       ), 
                              show_legend=T)
...
saveRDS(mat_red, file.path(path, "data/mat_red_int-1px-mean.rds"))
...
for(i in 1: length(samples)){
  ...
  ggsave(filename=file.path(path, paste0("data/UMAP_single_cluster_", samples[i] , ".tiff")), width = 10, height = 5, units = "in", device='tiff', dpi=500)
}
...
write.csv(data_wide, file.path(path, "/data/clusters-per-sample.csv"))
...
write.csv(data_wide, file.path(path, "/data/samples-per-cluster.csv"))
