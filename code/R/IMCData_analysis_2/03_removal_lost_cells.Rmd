...
intensity_mat_bg_corrected <- intensity_mat_mean_80[!grepl("2IF_GD2|3IF_CD56|DAPI", rownames(intensity_mat_mean_80)),]
...
```
We are excluding 3683 cells based on morphology and 8123 cells based due to zero intensity. 
...
saveRDS(clusters, file.path(path, "data/clusters-int-no-dil-mean80_identify-lost-cells.rds"))
...
```
We can see that cluster 22 are most probably the sticky cells, which are positive for everything. Cluster 22 are most probably cells that swam off between IF and IMC and are hence negative for everything except for DAPI. We will check which sample has the highest presence of this cluster.
