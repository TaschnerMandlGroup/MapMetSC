...
mat_red <- calculateUMAP(mat, ncomponents=5, n_neighbors=30, min_dist= 0, BPPARAM = MulticoreParam())
rownames(mat_red) <- colnames(spe)

saveRDS(mat_red, file.path(path, "results/mat_red_5components.rds"))

# mat_red <- readRDS(file.path(path,"results/mat_red_final.rds"))
# rownames(mat_red) <- colnames(spe)

#Apply changes to SPE object
reducedDim(spe, "UMAP_5") <- mat_red
```
...
saveRDS(clusters, file.path(path, "results/clusters_UMAP-5-dim.rds"))
...
for(i in 1: length(unique(clusters))){
  ...
  png(file=file.path(path, paste0("results/dim_reduction/UMAP_phenograph/heatmaps/", c , ".tiff")),width=18,height=8,units="in", res=1200)
