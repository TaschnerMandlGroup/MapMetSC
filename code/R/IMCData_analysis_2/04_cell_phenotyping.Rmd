...
# 
# saveRDS(clusters, file.path(path, "results/clusters_final.rds"))
...
# 
# saveRDS(mat_red, file.path(path, "results/mat_red_final.rds"))
...
for (i in 1: nrow(mat_tmp)){
  ...
  ggsave(filename=file.path(path, paste0("results/UMAP/markers/", marker , ".tiff")), width = 10, height = 5) #units = "in", device='tiff', dpi=500)
} 
```
...
for(i in 1: length(samples)){
  ...
  ggsave(filename=file.path(path, paste0("results/UMAP/cluster/cluster_", samples[i] , ".tiff")), width = 10, height = 5, units = "in", device='tiff', dpi=500)
}
