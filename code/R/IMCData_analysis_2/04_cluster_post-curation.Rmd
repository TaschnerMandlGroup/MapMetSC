...
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20230112_DIMR_Ilastik/20230523_sc_data_combined"

spe <- readRDS(file.path(path,"spe_1px_curated.rds"))
...
We will then save morphological features in a matrix.
...
morph_mat_scaled <- t(apply(morph_mat, 1, clipScaler, upper=0.999, lower=0.001, min=0, max=1))
#morph_mat_scaled <- log(morph_mat_scaled + 0.1)
colnames(morph_mat_scaled) <- spe$sample

#tissue-specific scaling
morph_mat_PT <- as.matrix(t(colData(spe)[spe$tissue=="PT",morph_names]))
morph_mat_BM <- as.matrix(t(colData(spe)[spe$tissue=="BM",morph_names]))

rownames(morph_mat_PT) <- gsub('-0','',rownames(morph_mat_PT))
rownames(morph_mat_BM) <- gsub('-0','',rownames(morph_mat_BM))
#morph_mat_PT_scaled <- t(apply(morph_mat_PT, 1, clipScaler, upper=0.999, lower=0.001, min=0, max=1))
#morph_mat_BM_scaled <- t(apply(morph_mat_BM, 1, clipScaler, upper=0.999, lower=0.001, min=0, max=1))
morph_mat_PT_scaled <- t(apply(morph_mat_PT, 1, RESCALE, to=0:1))
morph_mat_BM_scaled <- t(apply(morph_mat_BM, 1, RESCALE, to=0:1))

morph_mat_scaled_sep <- cbind(morph_mat_PT_scaled, morph_mat_BM_scaled)
colnames(morph_mat_scaled_sep) <- spe$sample
...
library(Matrix)
colnames(morph_mat_scaled) <- spe$sample
morph_pseudobulk <- aggregate(t(morph_mat_scaled), list(colnames(morph_mat_scaled)), mean)
...
mat <- t(rbind(intensity_mat_hq_wo_zeros, "soldity"=morph_mat_scaled["solidity",], "area"=morph_mat_scaled_sep["area",]))

out <- FastPG::fastCluster(data=mat, k=45, num_threads=n_threads)
clusters <- factor(out$communities)

saveRDS(clusters, file.path(path, "intensity_solidity-scaled_area-scaled-sep/data/clusters-int-1px-mean_postcuration.rds"))
...
```
We can see that cluster 34 are the sticky cells, which are positive for everything. We will exclude this cluster and re-generate the heatmap.
...
saveRDS(mat_red, file.path(path, "intensity_solidity-scaled_area-scaled-sep/data/mat_red_int-1px-mean.rds"))

#mat_red <- readRDS(file.path(path,"data/mat_red_int-1px-mean.rds"))
```
...
int_mat <- intensity_mat_hq_wo_zeros
colnames(int_mat) <- colnames(spe)
mat_tmp <- rbind(morph_mat_scaled, int_mat)
...
for (i in 1: 14){
  ...
  ggsave(filename=file.path(path, paste0("intensity_solidity-scaled_area-scaled-sep/data/marker/UMAP_single_marker_", marker , ".tiff")), width = 10, height = 5) #units = "in", device='tiff', dpi=500)
} 
```
...
for(i in 1: length(samples)){
  ...
  ggsave(filename=file.path(path, paste0("data/cluster/UMAP_single_cluster_", samples[i] , ".tiff")), width = 10, height = 5, units = "in", device='tiff', dpi=500)
}
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(path, paste("/data/sample/UMAP_1px-mean80_single_sample_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
...
write.csv(data_wide, file.path(path, "intensity_solidity-scaled_area-scaled-sep/data/clusters-per-sample.csv"))

data_long <- data_wide[126:175,] %>% pivot_longer(cols = !sample, names_to = "cluster", values_to = "count") 
...
ggplot(data_long, aes(fill=cluster, y=count, x=sample)) + 
  ggtitle("Clusters per BM sample 101-150") +
...
write.csv(data_wide, file.path(path, "intensity_solidity_aspect-ratio/data/samples-per-cluster.csv"))
...
#write.csv(data_wide, file.path(path, "/data/tissues-per-cluster.csv"))
