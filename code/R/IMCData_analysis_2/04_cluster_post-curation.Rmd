...
spe <- spe[,non_zeros]
rowData(spe)$use_channel <- !grepl("mean-80|IDO|DAPI|IF2_GD2|IF3_CD45/56|DNA1|H4K12Ac|H3K9Ac|CXCR2|PNMT|Fibro", rownames(spe))
...
for(i in 1: length(files)){
  ...
  plotCells(masks,
            ...
            display = "single",
            save_plot = list(filename = file.path(path, "results/int_morph/max_sample_per_cluster", paste0("all_", files[i], "f"))),
            legend = NULL)
  ...
  #           display="single",
  #           save_plot = list(filename = file.path(path, "results/int_morph/max_sample_per_cluster", paste0(files[i], "f"))),
  #           legend=NULL)
