...
```{r barplot-cluster-per-sample, fig.height=15, fig.width=4}
 # library
library(ggplot2)
library(tidyverse)
...
col_clusters <- metadata(spe_reduced)$col_clusters
clusters <- spe_reduced$pg_clusters 

col_clusters <- col_clusters[order(as.integer(names(col_clusters)))]
data <- data.frame(sample = group, clusters)
data$freq <- 1

data_wide <- data %>% pivot_wider(names_from = c("clusters"), values_from = "freq", values_fn = sum, values_fill = 0)

write.csv(data_wide, file.path(path, paste0("results/int_morph/clusters-per-", groupname, ".csv")))
...
ggplot(data_long, aes(fill=cluster, y=count, x=sample)) + 
  ggtitle(paste0("Clusters per ", groupname)) +
