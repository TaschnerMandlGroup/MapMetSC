...
```{r cell type umap, fig.width=15, fig.height=10, message=FALSE}
library(dittoSeq)
library(scater)
library(patchwork)
library(cowplot)
...
```{r cell type umap 2, fig.width=8, fig.height= 14, message=FALSE}
...
plot_grid(plotlist = plot_list, ncol=4, nrow=7)
...
```{r barplot celltype, fig.height=15, fig.width=5, message=FALSE}
# by sample_id
dittoBarPlot(spe, var = "celltype", group.by = "sample", retain.factor.levels=T) +
    scale_fill_manual(values = metadata(spe)$color_vector$col_celltype)

# by sample_id -count
dittoBarPlot(spe, var = "celltype", scale = "count", group.by = "sample", retain.factor.levels=T) +
    scale_fill_manual(values = metadata(spe)$color_vector$col_celltype)

# by patient_id - percentage
dittoBarPlot(spe, var = "celltype", group.by = "study_id", retain.factor.levels=T) +
    scale_fill_manual(values = metadata(spe)$color_vector$col_celltype)
...
dittoBarPlot(spe[,spe$tissue=="BM"], var = "celltype", group.by = "control", retain.factor.levels=T) +
    scale_fill_manual(values = metadata(spe)$color_vector$col_celltype)

# by infiltration - count
dittoBarPlot(spe[,spe$tissue=="BM"], scale = "count", var = "celltype", group.by = "control", retain.factor.levels=T) +
    scale_fill_manual(values = metadata(spe)$color_vector$col_celltype)
