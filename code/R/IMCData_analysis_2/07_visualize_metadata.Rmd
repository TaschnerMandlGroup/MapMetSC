...
```{r barplot-cluster-per-sample, fig.height=15, fig.width=5}
 # library
library(ggplot2)
library(tidyverse)

#change according to group of interest
# spe_reduced <- spe[,spe$tissue=="BM" & spe$control=="no"]
# groupname <- "timepoint"

spe_reduced <- spe[,spe$tissue=="BM"]
groupname <- "control"

# spe_reduced <- spe[,spe$timepoint=="DE"]
# groupname <- "tissue"

group <- colData(spe_reduced)[,groupname]

col_celltype <- metadata(spe_reduced)$color_vector$col_celltype
...
#if compartment specific visualization
compartment <- grepl("Tumor", colnames(data_wide))
data_wide <- data_wide[, compartment]
compartment <- grepl("Tumor", data_long$celltype)
data_long <- data_long[compartment,]
