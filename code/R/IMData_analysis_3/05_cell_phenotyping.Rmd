...
Rphenograph_out <- Rphenoannoy(t(mat), k = 45) # when performing tissue-separate clustering
...
```{r visualize-clusters-in-image, message=FALSE, fig.width=15}
library(dplyr)
annotation <- read.csv(file.path(path, "results", "in_house", "annotation_v4.csv"), sep=";")
...
# BM_cluster_names <- setNames(annotation$BM.level.1, annotation$cluster)
# PT_cluster_names <- setNames(annotation$PT.level.1, annotation$cluster)
...
set.seed(230820)
col_clusters <- setNames(sample(col_vector_433, length(unique(spe$celltype))), unique(spe$celltype))
metadata(spe)$color_vectors$col_celltype <- col_clusters

#metadata(spe)$color_vectors$col_BM_celltype <- setNames(unname(col_clusters), BM_cluster_names)
#metadata(spe)$color_vectors$col_PT_celltype <- setNames(unname(col_clusters), PT_cluster_names)
#metadata(spe)$color_vectors$col_celltype <- c(metadata(spe)$color_vectors$col_BM_celltype, metadata(spe)$color_vectors$col_PT_celltype)
```
We will do the same for the metaclusters.

```{r visualize-clusters-in-image, message=FALSE, fig.width=15}

BM_metacluster_names <- setNames(annotation$BM.level.2, annotation$cluster)
PT_metacluster_names <- setNames(annotation$PT.level.2, annotation$cluster)
...
n <- length(unique(spe$metacluster))
col_metacluster <- brewer.pal(n, "Set3")

metadata(spe)$color_vectors$col_metacluster <- setNames(col_metacluster[1:length(unique(spe$metacluster))], unique(spe$metacluster))
...
saveRDS(spe, file.path(path, "results/in_house/spe_clustered_3.rds"))
```
