...
spe <- readRDS(file.path(path,"spe_1px-curated.rds"))
...
mat_red_clustering <- readRDS(file.path(path,"mat_red_clustering.rds"))
...
clusters <- readRDS(file.path(path, "pg_clusters.rds"))
...
mat_red <- readRDS(file.path(path,"mat_red.rds"))
...
```{r visualize-clusters-in-image, message=FALSE, fig.width=15}
library(dplyr)
annotation <- read.csv(file.path(path, "annotation_v9.csv"), sep=";")
...
saveRDS(spe, file.path(path, "spe_clustered.rds"))
```
