...
spe <- readRDS(file.path(path,"results/spe_1px-curated.rds"))
...
```{r visualize-clusters-in-image, message=FALSE, fig.width=15}
library(dplyr)
annotation <- read.csv(file.path(path, "results", "annotation.csv"), sep=";")
BM_cluster_names <- setNames(paste0(annotation$cluster, "_", annotation$BM.annotation), annotation$cluster)
PT_cluster_names <- setNames(paste0(annotation$cluster, "_", annotation$PT.annotation), annotation$cluster)
...
col_metacluster <- brewer.pal(n,"Set1")

metadata(spe)$color_vectors$col_metacluster <- setNames(col_metacluster, unique(spe$metacluster))
