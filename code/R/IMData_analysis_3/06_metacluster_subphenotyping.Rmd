...
#tumor_clusters <- spe[, spe$metacluster=="tumor"]$celltype_fine
#granulocyte_clusters <- spe[, spe$metacluster=="granulocyte" & spe$tissue=="BM"]$celltype_fine
```

We will evaluate the results in single-cell and mean-per-cluster heatmaps. 

```{r single-cell-heatmap-clusters, message=FALSE, fig.width=15}
...
```
```{r ingle-cell-heatmap-clusters-mean, message=FALSE, fig.width=15}
library(scuttle)
library(bruceR)
...
Heatmap(mean_t, 
        ...
        # clustering_distance_columns="pearson", #use euclidian, kendal or pearson
        # clustering_method_columns="single",
        column_order= order(unname(mean_t["solidity",]), decreasing=F),
        cluster_rows=T,
        #show_column_dend = T,
        show_column_names=T,
        show_row_names=T,
...
#calculate mean heatmap over all cells
matrix <- granulocyte_mat[,granulocyte_clusters != "gran_25"]
clust <- granulocyte_clusters[granulocyte_clusters != "gran_25"]
...
mat <- granulocyte_mat 
colnames(mat) <- spe[,colnames(granulocyte_mat)]$sample_id
...
data_wide <- as.data.frame(t(data_wide[,-1]))
colnames(data_wide) <- n

write.csv(data_wide, file.path(path, "results/in_house/metadata/image_per_celltype_gran.csv"), row.names=T)
```
...
files <- list.files(file.path(path, "results/in_house/colored_masks/granulocyte/masks_temp", tissue))

for(i in 1: length(files)){
  
  masks <- loadImages(file.path(path, "results/in_house/colored_masks/granulocyte/masks_temp", tissue, files[i]), as.is = TRUE)
  ...
  plotCells(masks,
            ...
            display="single",
            save_plot = list(filename = file.path(path, "results/in_house/colored_masks/granulocyte/max_sample_per_cluster", tissue, paste0(files[i], "f"))),
            legend=NULL)
