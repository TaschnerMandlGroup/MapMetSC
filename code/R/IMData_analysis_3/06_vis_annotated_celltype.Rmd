...
data_long <- data_long[grepl("BM", data_long$sample) & (grepl("MYCN:1", data_long$sample)| grepl("MYCN:3", data_long$sample)),]
...
ggplot(data_long, aes(fill=celltype, y=count, x=sample)) + 
  ggtitle("Clusters per BM sample") +
...
for(i in 1: length(files)){
  ...
  plotCells(masks,
            ...
            display = "single",
            save_plot = list(filename = file.path(path, "results/summary", paste0("all_", files[i], "f"))),
            legend = NULL)
...
```{r mean-per-cluster-heatmap-with-metadata, message=FALSE, fig.height=20, fig.width=40}
