...
df <- d$data
```
The problem above is that the internal scaling can't be turned off. So we will try to plot a dotplot with complex heatmap. 

```{r dot-plot-complex-heatmap, message=FALSE, fig.height=8, fig.width=12}
library(tidyr)
library(bruceR)
...
column_ha = HeatmapAnnotation(metacluster=mean_metaclust$x,
                              ...
                              ncells=anno_barplot(as.data.frame(table(spe$celltype))[,"Freq"], height = unit(1, "cm")),
                              col=list(metacluster=metadata(spe)$color_vectors$col_metacluster, 
                                       celltype=metadata(spe)$color_vectors$col_celltype
                                       ), 
                              show_legend=F)
...
col_fun = circlize::colorRamp2(c(0, 0.5, 1), viridis(100)[c(1,50,100)])
