...
spe <- readRDS(file.path(path,"results/in_house/spe_clustered_6.rds"))
```
...
for(i in 1: length(unique(celltype))){
  ...
  png(file=file.path(path, paste0("results/in_house/heatmaps/merged/clusters-tissue-sep/", c , ".tiff")),width=18,height=8,units="in", res=1200)
...
```{r dot-plot-complex-heatmap, message=FALSE, fig.height=11, fig.width=16}
library(tidyr)
library(bruceR)
...
column_ha = HeatmapAnnotation(metacluster=mean_metaclust$x,
                              ...
                              solidity = solidity_mean,
                              area = area_mean,
                              ncells_overall=anno_barplot(as.data.frame(table(spe$celltype))[,"Freq"], add_numbers=T, height = unit(1.8, "cm")),
                              ncells_PT=anno_barplot(unname(pt_count), add_numbers=TRUE, height = unit(1.8, "cm"), gp = gpar(fill = "brown")),
                              ncells_bm=anno_barplot(unname(bm_count), add_numbers=TRUE, height = unit(1.8, "cm"), gp = gpar(fill = "aquamarine4")),
                             col=list(metacluster=metadata(spe)$color_vectors$col_metacluster, 
                                       celltype=metadata(spe)$color_vectors$col_celltype,
                                       solidity = col_mean_solidity,
                                        area = col_mean_area
...
cell_fun = function(j, i, x, y, w, h, fill){
          ...
          grid.circle(x=x,y=y,r= percent_mat[i, j]/1.3 * min(unit.c(w, h)),
...
layer_fun = function(j, i, x, y, w, h, fill){
          grid.rect(x = x, y = y, width = w, height = h, 
                    gp = gpar(col = NA, fill = NA))
          grid.circle(x=x,y=y,r= pindex(percent_mat, i, j)/1.3 * unit(2, "mm"),
...
write.csv(df, file.path(path, "results/in_house/metadata/tumor_infiltration_IF_vs_IMC.csv"))
```
...
for(i in 1: length(files)){
  ...
  #masks <- loadImages(file.path(path, "results/in_house/colored_masks/masks_temp", tissue, files[i]), as.is = TRUE)
