...
code <- setNames(protein2gene$protein_triana, protein2gene$protein)
...
common_features <- intersect(row.names(reference2), row.names(protein))
...
library(paletteer)
set.seed(230713)
reference2@active.ident <- reference2@meta.data$ct
col_vector_433 = colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
col_reference_clusters = setNames(sample(col_vector_433, length(unique(reference2@active.ident))), unique(reference2@active.ident))

DimPlot(reference2, reduction="ProjectedUMAP", cols=col_reference_clusters)
