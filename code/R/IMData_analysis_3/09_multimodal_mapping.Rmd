...
protein_sce <- protein_sce[,protein_sce$metacluster=="tumor" | protein_sce$celltype=="78_schwann cells" | protein_sce$celltype =="78_SCP"]
protein_sce$predicted_tumor_clusters <- predicted_labels$labels
saveRDS(protein_sce, file.path(path,"public_datasets/Jansky/results/spe_tumor_clusters.rds"))

#plotScoreHeatmap(predicted_labels[sample(seq_len(nrow(predicted_labels)), 16000),])
plotScoreHeatmap(predicted_labels, cluster_cols=col_predicted_labels)
```
...
write.csv(data_wide, file.path(path,"public_datasets/Jansky/results/predicted_RNA_per_protein_cluster.csv"))
...
write.csv(assignments, file.path(path,"public_datasets/Jansky/results/similarity_assignments_tumor.csv"))
```
