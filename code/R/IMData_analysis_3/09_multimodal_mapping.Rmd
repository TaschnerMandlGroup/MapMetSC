...
rna <- readRDS(file.path(path,"public_datasets/Lee/disco_bone_marrow_v2.0.rds"))
...
protein <- protein[,protein@meta.data$metacluster!="tumor" & protein@active.ident!="53_sticky cells"]
...
saveRDS(mat_red_df, file.path(path,"public_datasets/Jansky/results/mat_red_NBonly.rds"))
```

We will then visualize the integrated data in the UMAP space for RNA and protein separately. 

```{r batch-correction and dimensionality reduction, fig.width=15, message=FALSE}
mod <- "protein"
...
plot <- ggplot(background, aes(x=V1, y=V2)) +
    ...
    geom_point(data=foreground, aes(x=V1, y=V2, colour=clusters), show.legend=F, size=0.2) +
    scale_colour_manual(values = col_clust) +
...
mean <- aggregate(mat_integrated, list(clusters), mean)
...
modality <- rep("rna", nrow(data))
modality[unlist(lapply(rownames(data), FUN=function(x){grepl("_",x, fixed = TRUE)}))] <- "protein"
...
We will re-compute UMAP for the protein data in order to obtain a UMAP model and then project the scRNAseq data onto the generated UMAP using [Seurat](https://satijalab.org/seurat/reference/projectumap). 
...
RunUMAP(object = protein, dims = 1:dim(protein)[2], return.model=T)
...
```{r single-r, fig.height=20, fig.width= 15, message=FALSE}
library(BiocParallel)
library(SingleR)
...
predicted_labels <- SingleR(test = protein_mat, ref = rna_mat, labels = rna@active.ident, BPPARAM = MulticoreParam())

protein_sce <- readRDS(file.path(path,"results/spe_clustered.rds"))
protein_sce <- protein_sce[,protein_sce$metacluster!="tumor" & protein_sce$celltype!="53_sticky cells"]
protein_sce$predicted_immune_clusters <- predicted_labels$labels
saveRDS(protein_sce, file.path(path,"public_datasets/Lee/spe_immune_clusters.rds"))
...
rna_mat <- rna_mat <- GetAssayData(rna, assay = "RNA", slot = "data")[c(common_features, additional_NE_markers), ]
...
col_true_labels = setNames(sample(col_vector_74, length(unique(true_label))), unique(true_label))
...
Heatmap(as.matrix(rna_mat[,rand_cells]), 
...
```{r mean-per-cluster-protein, fig.height=12, fig.width=8, message=FALSE}
library(bruceR)
rna_mean <- aggregate(t(rna_mat), list(predicted_labels$labels), mean)
...
```{r evaluate-barplot, fig.width=14, message=FALSE}

data <- data.frame(protein_clusters=predicted_label, rna_clusters=rna@active.ident)
...
ggplot(data_long, aes(fill=rna_clusters, y=count, x=protein_clusters)) + 
  ggtitle("RNA clusters per predicted protein cluster") +
  geom_bar(position="fill", stat="identity", show.legend = T) + 
  scale_fill_manual(values = col_true_labels) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```
