...
#rna <- readRDS(file.path(path,"results/public_datasets/Jansky/adrenal_medulla_Seurat.RDS"))

#only for Fetahu et al.
monocle_cell_dataset <- readRDS(file.path(path,"results/public_datasets/Fetahu/rna_integrated_monocle.rds"))
...
protein <- readRDS(file.path(path,"results/in_house/spe_clustered_8.rds"))
...
#rna <- subset(x = rna, subset=fate%in%c("sympathoblasts", "chromaffin", "SCP"))
```
...
```{r single-r, fig.height=3, fig.width= 10, message=FALSE}
...
saveRDS(spe, file.path(path, "results/public_datasets/Fetahu/20231030_spe_predicted.rds"))
```
...
tiff(filename = file.path(path, "results/in_house/20231030/UMAP_predicted_celltype_fetahu.tiff"),
...
tiff(filename = file.path(path, "results/in_house/20231030/UMAP_predicted_metacluster_fetahu.tiff"),
     width = 115, height = 95, units = "mm", res=600)
p1
dev.off()
