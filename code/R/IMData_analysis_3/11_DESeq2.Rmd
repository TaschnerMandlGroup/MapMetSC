...
spe <- spe[,spe$progression=="BM" & spe$timepoint=="DE"]
...
gg_df <- left_join(gg_df, ei[, c("sample_id", "progression", "study_id")]) 


metadata <- gg_df %>%
        dplyr::select(cluster_id, sample_id, progression, study_id) 
...
cluster_metadata$condition <- factor(cluster_metadata$progression)
#cluster_metadata$study_id <- factor(cluster_metadata$study_id)
...
dds$condition <- relevel(dds$condition, ref = "0") #https://www.biostars.org/p/448959/ #change if other condition used, e.g. "BM"
# resultsNames(dds)
res <- results(dds, 
               contrast = contrast,
               alpha = 0.05)
...
gathered_top10_sig <- inner_join(ei[, c("sample_id", "progression" )], gathered_top10_sig, by = c("sample_id" = "samplename"))

## plot using ggplot2
ggplot(gathered_top10_sig) +
        geom_point(aes(x = gene, 
                       y = normalized_counts, 
                       color = progression), #Here as well!!
...
get_dds_resultsAvsB <- function(x, A, B){
        ...
        dds$condition <- relevel(dds$condition, ref = "0") #https://www.biostars.org/p/448959/
        ...
        gathered_top20_sig <- inner_join(ei[, c("sample_id", "progression" )], gathered_top20_sig, by = c("sample_id" = "samplename")) #change for other condition
        
        ## plot using ggplot2
        ggplot(gathered_top20_sig) +
                geom_point(aes(x = gene, 
                               y = normalized_counts, 
                               color = progression), #change for other condition
                           position=position_jitter(w=0.1,h=0)) +
                scale_y_log10() +
                xlab("Genes") +
                ylab("log10 Normalized Counts") +
                ggtitle(paste0("Top 20 Significant DE Genes in ", clusters[x])) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
                theme(plot.title = element_text(hjust = 0.5))
