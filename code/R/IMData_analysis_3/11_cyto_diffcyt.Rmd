...
sce$sample_id <- factor(spe[keep_rows, keep_cols]$sample)
...
# spe_cat <- spe[rowData(spe)$use_channel,]
# 
# spe_cat$sample <- factor(spe$sample)
# spe_cat$sample_id <- factor(spe$sample_id)
# spe_cat$condition <- factor(spe$timepoint)
...
plotDR(sce, "UMAP", color_by = "CD4_Yb171_mean")
```
...
```{r umap-facet-sample, fig.height = 6, fig.cap = "UMAP as in Figure \\@ref(fig:umap-one-clustering1), but stratified by sample.", fig.width=15, fig.height=20}
# facet by sample
plotDR(sce, "UMAP", color_by = "celltype", facet_by = "sample_id")
```

```{r umap-facet-condition, fig.height = 3.5, fig.cap = "UMAP as in Figure \\@ref(fig:umap-one-clustering1), but stratified by condition.", fig.width=10, fig.height=10}
# facet by condition
...
```{r diff-freqs-plot-props-barplot, fig.cap = "Relative abundance of the 8 PBMC populations in each sample (x-axis), in the PBMC dataset, represented with a barplot. The 8 cell populations are a result of manual merging of the 20 FlowSOM metaclusters.", fig.width=50, fig.height=20}
...
#     cols_fixed = "condition", cols_random="sample_id")

# (da_formula2 <- createFormula(ei,
#     cols_fixed = "timepoint",
#     cols_random = c("sample", "study_id")))
```
...
da_res1 <- diffcyt(sce, 
    ...
    clustering_to_use = "celltype", min_cells=1, min_samples=1, verbose = T)
```
