...
paired_patients <- rownames(meta_timepoint)[meta_timepoint$BM_DE==1 & meta_timepoint$BM_REL==1]
```
...
keep_cols <- spe$tissue=="BM" & spe$control=="no" & spe$metacluster=="T-cell" & (spe$timepoint=="DE" | spe$timepoint=="RE2") #& spe$study_id%in%paired_patients
#keep_cols <- spe$tissue=="BM" & (spe$timepoint=="DE" | spe$timepoint=="") & spe$metacluster != "tumor"
...
sce$cluster_id <- factor(spe[keep_rows, keep_cols]$celltype_merged)
...
metadata(sce)$cluster_codes <- data.frame(celltype = factor(spe[keep_rows, keep_cols]$celltype_merged))
...
```{r diff-freqs-plot-props-barplot, fig.cap = "Relative abundance of the 8 PBMC populations in each sample (x-axis), in the PBMC dataset, represented with a barplot. The 8 cell populations are a result of manual merging of the 20 FlowSOM metaclusters.", fig.width=30, fig.height=10}
...
# 
# nMNA.RE2 <- mna == "nMNA" & condition=="RE2"
# MNA.RE2 <- mna == "MNA" & condition=="RE2"
# unknown.RE2 <- mna == "unknown" & condition=="RE2"
# 
...
# res <- glmQLFTest(fit.ab, coef="MNA.RE2")
