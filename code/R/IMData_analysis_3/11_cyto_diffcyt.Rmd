...
plotDR(sce, "UMAP", color_by = "CD44_In115_mean")
```
...
```{r}
#design <- model.matrix(~factor(sample_id) + factor(condition), y.ab$samples)
...
```
We test for differences in abundance between timepoints using `glmQLFTest()`.
...
To understand this result, will next look at a summary of normalized abundances.
```{r}
library(tidyr)
normalized_proportions <- as.data.frame(t(res$fitted.values))
normalized_proportions$condition <- y.ab$samples$condition

norm_prop_long <- normalized_proportions %>%
  pivot_longer(!condition, names_to = "cluster", values_to = "mean")

abundance_table_normalized <- as.data.frame(norm_prop_long %>% group_by(condition, cluster) %>% 
