...
sce_bblab <- prepData(fs, panel, md, features = panel$fcs_colname)
```
...
p <- plotExprs(spe_reduced, color_by = "timepoint")
...
n_cells(sce) 
plotCounts(sce, group_by = "sample", color_by = "condition")
```
...
pbMDS(sce, by="sample_id", color_by = "condition")
```
...
plotAbundances(spe_reduced, k = "celltype", by = "sample")
```
