...
spe <- readRDS(file.path(path,"results/in_house/spe_clustered_9.rds"))
...
BM_patients_3 <- unique(spe[,spe$tissue=="BM" & spe$control=="no" & spe$metacluster!="tumor" & spe$timepoint == "REL"]$study_id)
...
keep_cols <- spe$celltype != "other" & spe$tissue=="PT" #spe$metacluster !="tumor" & spe$metacluster !="mesenchymal"
...
vec2<- paste0(unique(spe[keep_rows, keep_cols]$fm_id),"_PT")
index_in_vec1 <- match(vec2, vec1)
fm_id <- vec2[order(index_in_vec1)]
sce$sample_id <- factor(paste0(spe[keep_rows, keep_cols]$fm_id, "_PT"), levels=fm_id)
...
```{r diff-freqs-plot-props-barplot, fig.cap = "Relative abundance of the 8 PBMC populations in each sample (x-axis), in the PBMC dataset, represented with a barplot. The 8 cell populations are a result of manual merging of the 20 FlowSOM metaclusters", fig.width=28, fig.height=12}
sce_ME <- sce[,sce$metacluster!="tumor" & sce$metacluster!="mesenchymal"]

set.seed(20231103)
p <- plotAbundances(sce_ME, k = "celltype", by = "sample", col_clust=F, k_pal=metadata(spe)$color_vectors$col_celltype_ordered, 
...
cellular_communities <- as.data.frame(cutree(hc, k = 4)[levels(p$data$sample_id)])
...
setEPS()
postscript(file.path(path, "results/in_house/20231115/dendogram_all_samples.eps"), height=12, width=30)
plot(hc)
dev.off()

setEPS()
postscript(file.path(path, "results/in_house/20231115/barplot_all_samples.eps"), height=12, width=20)
p
dev.off()
```
...
```{r plot-tumor-compositions, message=FALSE, fig.width=28, fig.height=12}
clustered_samples <- as.vector(unique(sce$sample_id)) # when subsetting the dataset
...
setEPS()
postscript(file.path(path, "results/in_house/20231115/barplot_all_samples_tumor.eps"), height=12, width=20)
p_tu
dev.off()
```

As a next step, we will compute the heterogeneity of samples based on the ME and tumor cell composition.
```{r heterogeneity, message=FALSE, fig.height=25}
library(BioQC)
...
meta_path <- file.path(path, '20231127_metadata_complete.csv')
...
setEPS()
postscript(file.path(path, "results/in_house/20231115/annotationbar_all_samples.eps"), height=30, width=30)
...
#absolute cellcount
setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/annotationbar_all_samples_cells.eps")), height=30, width=30)
draw(anno_barplot(as.vector(unname(table(sce$sample_id))), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/annotationbar_all_samples_tumorcells.eps")), height=30, width=30)
...
setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/annotationbar_all_samples_tumor_inf.eps")), height=30, width=30)
draw(anno_barplot(as.vector(tumor_infiltration), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/annotationbar_all_samples_tumor_inf_norm.eps")), height=30, width=30)
...
```{r summary_plots-dis, message=FALSE, fig.height=20}
library(gridExtra)
...
discrete_metadata <- c("control", "Tissue", "Timepoint", "MYCN_amp", "ALK_amp", "ALK_mut", "ATRX_del", "TERT_rear", "X1p_loss", "X1q_gain", "X11q_loss", "X17q_gain", "efs", "therapy")

col_timepoint = setNames(c('#beaed4', '#386cb0', '#386cb0', '#ffff99'), c("DE", "RE1", "RE2", "REL")) 
#col_aberrations = setNames(c('#a6611a','#018571','#80cdc1','#f5f5f5'), c(0,1,3,9))

discrete_metadata_colors <- list(col_control, col_tissue, col_timepoint, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_progress, col_therapy)
...
```{r summary_plots-con, message=FALSE, fig.height=20, fig.width=5}
library("ggpubr")
