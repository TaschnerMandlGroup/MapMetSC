...
samples_with_low_tumor_inf <- unique(spe[,spe$tissue=="BM" & spe$metacluster=="tumor"]$sample)[unname(table(spe[,spe$tissue=="BM" & spe$metacluster=="tumor"]$sample))<=10]

#spe$tmp <- "na"

#binary variable for low and high tumor cell infiltration
spe$tmp <- "high"
spe[,spe$sample%in%samples_with_low_tumor_inf | !spe$sample%in%samples_with_tumor_cells]$tmp <- "no/low"

keep_rows <- rowData(spe)$use_channel 
keep_cols <- spe$celltype != "other" & spe$tissue=="BM" & (spe$timepoint=="DE" | spe$timepoint=="") #& spe$sample_id%in%samples_with_tumor_cells   #spe$sample_id%in%samples_with_tumor_cells & #& (spe$MYCN_amp==0 | spe$MYCN_amp==1) # # (spe$timepoint=="DE" | spe$timepoint=="RE2") & spe$study_id%in%paired_patients & spe$sample_id%in%samples_with_tumor_cells 
