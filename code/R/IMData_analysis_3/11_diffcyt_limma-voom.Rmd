...
keep_cols <- spe$celltype != "other" #spe$metacluster !="tumor" & spe$metacluster !="mesenchymal"
...
set.seed(20231103)
p <- plotAbundances(sce_ME, k = "celltype", by = "sample", col_clust=T, k_pal=metadata(spe)$color_vectors$col_celltype_ordered, 
...
discrete_metadata <- c("control", "Tissue", "Timepoint", "MYCN_amp", "ALK_amp", "ALK_mut", "ATRX_del", "TERT_rear", "X1p_loss", "X1q_gain", "X11q_loss", "X17q_gain", "efs")

col_timepoint = setNames(c('#beaed4', '#386cb0', '#386cb0', '#ffff99'), c("DE", "RE1", "RE2", "REL")) 
#col_aberrations = setNames(c('#a6611a','#018571','#80cdc1','#f5f5f5'), c(0,1,3,9))

discrete_metadata_colors <- list(col_control, col_tissue, col_timepoint, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_aberrations, col_progress)
...
```{r summary_plots-con, message=FALSE, fig.height=15, fig.width=5}
library("ggpubr")
...
cont_df <- data.frame(cbind(n_overall, n_tumor, perc_tumor, perc_tumor_norm, het_m, het_t, cellular_communities),row.names=clustered_samples)
