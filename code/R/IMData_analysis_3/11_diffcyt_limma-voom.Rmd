...
spe <- readRDS(file.path(path,"results/in_house/spe_clustered_10.rds"))
...
keep_cols <- spe$celltype != "other" & spe$tissue=="BM" #& spe$timepoint=="REL" #spe$metacluster !="tumor" & spe$metacluster !="mesenchymal"
...
setEPS()
postscript(file.path(path, "results/in_house/20231115/REL_ME_barplot.eps"), height=12, width=20)
p
dev.off()
```
...
setEPS()
postscript(file.path(path, "results/in_house/20231115/REL_TU_barplot.eps"), height=12, width=20)
p_tu
dev.off()
```

As a next step, we will compute the heterogeneity of samples based on the ME and tumor cell composition.
```{r heterogeneity, message=FALSE}
library(BioQC)
library(hrbrthemes)
...
setEPS()
postscript(file.path(path, "results/in_house/20231115/REL_annotation_bar.eps"), height=30, width=30)
...
#absolute cellcount
setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/REL_annotationbar_ncells.eps")), height=30, width=30)
draw(anno_barplot(as.vector(unname(table(sce$sample_id))), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/REL_annotationbar_ntumorcells.eps")), height=30, width=30)
...
setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/REL_annotationbar_tumor_inf.eps")), height=30, width=30)
draw(anno_barplot(as.vector(tumor_infiltration), add_numbers=T, height = unit(1, "cm")))
dev.off() 

setEPS()
postscript(file.path(path, paste0("results/in_house/20231115/REL_annotationbar_tumor_inf_norm.eps")), height=30, width=30)
