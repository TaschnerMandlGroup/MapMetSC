...
keep_cols <- spe$tissue=="BM" & spe$metacluster != "tumor" & spe$sample_id%in%samples_with_tumor_cells #& spe$control=="yes"
...
```{r diff-freqs-plot-props-barplot, fig.cap = "Relative abundance of the 8 PBMC populations in each sample (x-axis), in the PBMC dataset, represented with a barplot. The 8 cell populations are a result of manual merging of the 20 FlowSOM metaclusters", fig.width=25}
set.seed(20231103)
...
setEPS()
postscript(file.path(path, "results/in_house/20231030/dendogram_allBMsamples.eps"), height=12, width=30)
plot(hc)
dev.off()
