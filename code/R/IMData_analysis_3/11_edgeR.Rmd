...
spe_reduced <- spe_reduced[, spe_reduced$timepoint=="DE" | spe_reduced$timepoint=="RE2"]


```

```{r}
abundances <- table(spe_reduced$celltype, spe_reduced$sample) 
...
extra.info <- colData(spe_reduced)[match(colnames(abundances), spe_reduced$sample),]
...
```{r}
#design <- model.matrix(~factor(tissue) + factor(timepoint), y.ab$samples)
design <- model.matrix(~factor(timepoint), y.ab$samples)
deisgn <- createDesignM
```
