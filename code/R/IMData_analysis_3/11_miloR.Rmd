...
As a next step, we will test miloR on only the tumor cells to detect neighborhoods that are differentially abundant between samples from patients with and without progression.
...
# 
# sce <- spe[rownames(spe)%in%tumor_markers, spe$metacluster=="tumor" & spe$tissue == "PT"]
...
sce <- spe[rownames(spe)%in%tumor_markers, spe$metacluster=="tumor" & spe$tissue=="BM" & spe$timepoint=="DE"]
...
sce <- runPCA(sce, ncomponents=10, exprs_values="logcounts", BPPARAM = MulticoreParam(RNGseed = 221228))
...
```{r}
milo <- buildGraph(milo, k = 45, d = 10, reduced.dim = "PCA", BPPARAM = MulticoreParam(RNGseed = 221228))
```
...
```{r}
set.seed(20230820)
milo <- makeNhoods(milo, prop = 0.1, k = 45, d=10, refined = TRUE, reduced_dims = "PCA")
```
...
```{r}
design <- data.frame(colData(milo))[,c("sample", "study_id", "progression", "tissue", "timepoint")]
...
dittoDimPlot(sce, var = "celltype_fine", 
...
da_results <- annotateNhoods(milo, da_results, coldata_col = "celltype_fine")
head(da_results)
```
...
```{r}
da_results$celltype <- ifelse(da_results$celltype_fraction < 0.7, "Mixed", da_results$celltype_fine)
```

Now we can visualize the distribution of DA Fold Changes in different cell types

```{r, fig.height=7, fig.width=7, dev="jpeg"}
plotDAbeeswarm(da_results, group.by = "celltype_fine")
```
...
plotNhoodExpressionGroups(milo, da_results, #features=intersect(rownames(milo), markers[1:10]),
                          features=rownames(milo), 
                          subset.nhoods = da_results$NhoodGroup %in% c('4','5', '6'), 
                          scale=TRUE,
                          assay="logcounts",
                          grid.space = "fixed")
```
