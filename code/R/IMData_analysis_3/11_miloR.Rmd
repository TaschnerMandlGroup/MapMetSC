...
sce <- spe[rowData(spe)$use_channel, spe$metacluster=="tumor" & spe$tissue=="BM" & spe$timepoint=="DE"]
...
sce <- sce <- runUMAP(sce, dimred = "PCA", name = 'umap', BPPARAM = MulticoreParam(RNGseed = 221228))
...
```{r}
design <- data.frame(colData(milo))[,c("sample", "progression")]
