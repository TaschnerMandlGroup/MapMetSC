...
As a next step, we will test miloR on only the tumor cells to detect neighborhoods that are differentially abundant during different timepoints (crtl, DE, RE1, RE2, REL).
...
```{r}
rownames(design) <- design$sample
contrast.1 <- c("timepointPT_DE - timepointBM_DE") # the syntax is <VariableName><ConditionLevel> - <VariableName><ControlLevel>
```
...
da_results <- testNhoods(milo, design = ~ control, design.df = design, fdr.weighting="graph-overlap")
head(da_results)
