...
Using the new 2026 re-analysis Rds object here (LATEST VERSION AS OF 30.03.2026). 
...
CLUSTERS_OF_INTEREST  <- c("marker-lo TC", "Ki76hi TC") # SHOULD BE Ki67 not 76!!!!

ARTIFACT_CLUSTERS <- c("ELAVL4hi GATA3hi SOX10+ TC", "CXCR4hi TC",
                       "ELAVL4hi GATA3hi SOX10lo TC","CHGA- GD2hi CD56hi TC",
                       "Ki76hi TC")
...
# NOTE: When very few BM+DE units are flagged (n<30), most TC clusters will have
# pct_detected_flagged = NaN (cluster never seen in the one flagged unit).
...
artifact_cor_plot %>%
  ...
  labs(title = "Test 2b — Detection enrichment in flagged vs reliable units",
       subtitle = paste0("Positive = cluster detected more often in low-n samples = artifact signal\n",
...
zero_summary %>%
  ...
  labs(title = "Test 3 — 95% CI overlap with zero per cluster",
       subtitle = paste0("Among units passing strict detection threshold (≥", MIN_DETECT_CELLS,
                         ...
                         "in the single flagged BM+DE unit (Patient_044, n=12)"),
