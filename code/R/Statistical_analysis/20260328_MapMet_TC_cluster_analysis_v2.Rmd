...
Using the original analysis here (2024).
...
seu_meta_annot <- seu_meta %>%
  ...
  mutate(
    is_control = is.na(Timepoint) & Tissue == BM_LABEL,
    Timepoint  = ifelse(is_control, DE_LABEL, Timepoint)
  )
...
message("✓ seu_meta_annot: ", nrow(seu_meta_annot), " cells | ",
        ...
        sum(seu_meta_annot$is_control & !duplicated(seu_meta_annot$Sample)),
        " stage L control patients")

seu_meta_annot %>%
  distinct(Sample, Tissue, Timepoint, is_control) %>%
  count(Tissue, Timepoint, is_control) %>%
  mutate(note = ifelse(is_control, "stage L controls (no BM infiltration; assigned DE)", "")) %>%
  select(-is_control) %>%
  knitr::kable(caption = "Units (patient × tissue) per tissue × timepoint combination. Stage L BM controls are patients enrolled without detectable tumor infiltration; they carry no Timepoint entry in the metadata and are assigned to the DE group for analysis.")
```
