...
Using the new 2026 re-analysis Rds object here (LATEST VERSION AS OF 09.04.2026)

# Overview

This document presents a complete statistical analysis of tumor cell (TC) cluster reliability and tissue-specific enrichment in the MapMet IMC dataset. The dataset comprises imaging mass cytometry (IMC) acquisitions from primary tumors (PT) and bone marrow (BM) patient_ids across multiple timepoints from neuroblastoma patients.

The analysis addresses two central questions:

1. **Which TC cluster calls are statistically reliable**, given that some patient_ids — particularly BM patient_ids — contain very few TC cells? TC cluster proportions derived from small cell counts are subject to substantial multinomial sampling noise and may not reflect true biology.

2. **Are specific TC clusters genuinely enriched in BM relative to PT at diagnosis**, as claimed for marker-lo Tu and Ki67+ Tu cells?

**Study design note:** The cohort has a mixed paired/unpaired structure. A subset of patients contributed both a primary tumor (PT) and a bone marrow (BM) biopsy at the same timepoint — these paired patient_ids enable within-patient BM vs PT comparisons that control for inter-patient variability. The remainder contributed only one tissue type. The statistical models in section 5 account for this structure explicitly via a random intercept per patient.

The analysis proceeds in four stages: (1) bootstrap-based stability assessment of cluster calls, (2) a three-test artifact detection framework, (3) validation of flagged clusters in PT patient_ids, and (4) formal statistical testing of BM vs PT enrichment using multiple complementary models.
...
imc <- readRDS("/home/rstudio/mnt_out/MapMet/Rds/spe_final_PTBM_seurat.rds")
imc_metadata <- read.table(
  "/home/rstudio/mnt_out/MapMet/tables/spe_final_PTBM_metdata.csv",
  sep = ",", header = TRUE)
...
required_cols <- c("patient_id", "tissue", "celltype")
...
TC_PATTERN       <- "Tu|[Tt]umor"  # regex to identify Tu celltypes
...
Each cell in `seu_meta` carries a `patient_id` encoding the patient ID, tissue type, and acquisition number. The patient-level ID is extracted via regex and joined to the external metadata table to attach `tissue` and `Timepoint` annotations.

The join is performed on both `patient_id` (patient ID) and `tissue` (tissue type) simultaneously to prevent the many-to-many row explosion that occurs when a patient has patient_ids from multiple tissues — each cell is matched only to its own tissue's metadata entry.

A subset of BM patient_ids come from stage L patients (no tumor infiltration detected) who were enrolled as controls at the DE timepoint. These patient_ids have no explicit `Timepoint` entry in the metadata table; they are assigned `DE_LABEL` here before any downstream filtering.

```{r metadata-prep}
# name of TC clusters
grep(TC_PATTERN, unique(seu_meta$celltype), value = TRUE)

# sanity check
unique(seu_meta$tissue)
unique(seu_meta$patient_id)
...
The figure below shows the structure of the DE timepoint cohort — the cohort used for all BM vs PT enrichment analyses. Each row is one patient. Coloured tiles indicate which tissue types are available for that patient. Paired patients (with both a BM and a PT patient_id at DE) are shown first; stage L BM controls are indicated separately.
...
# Assign Patient IDs to control samples via fm_id
control_id_map <- seu_meta %>%
  filter(is_control, is.na(patient_id)) %>%
  distinct(fm_id) %>%
  arrange(fm_id) %>%
  mutate(Sample_ctrl = sprintf("Patient_%03d", seq(66L, length.out = n())))

seu_meta <- seu_meta %>%
  left_join(control_id_map, by = "fm_id") %>%
  mutate(patient_id = coalesce(patient_id, Sample_ctrl)) %>%
  select(-Sample_ctrl)

# Sanity checks
stopifnot("patient_id still NA after control ID assignment" = !any(is.na(seu_meta$patient_id)))
stopifnot("timepoint still NA after DE assignment"          = !any(is.na(seu_meta$timepoint)))

message("Control patients assigned: ",
        paste(sort(unique(seu_meta$patient_id[seu_meta$is_control])), collapse = ", "))
```

```{r cohort-figure, fig.height=10, fig.width=7}
de_cohort <- seu_meta %>%
  distinct(patient_id, tissue, control)
...
bm_only_patients <- de_cohort %>%
  filter(tissue == BM_LABEL & control == "no", !patient_id %in% paired_patients) %>%
  pull(patient_id) %>% unique()

pt_only_patients <- de_cohort %>%
  filter(tissue == PT_LABEL, !patient_id %in% paired_patients) %>%
  pull(patient_id) %>% unique()

control_patients <- de_cohort %>%
  filter(control == "yes", !patient_id %in% paired_patients) %>%
  pull(patient_id) %>% unique()
...
patient_order <- c(
  ...
  sort(control_patients)
)

cohort_df <- de_cohort %>%
  mutate(
    patient_id   = factor(patient_id, levels = rev(patient_order)),
    tile_label   = case_when(
      control == "yes"   ~ "BM (non-TU-infiltrated)",
      tissue == BM_LABEL ~ "BM (TU-infiltrated)",
      tissue == PT_LABEL ~ "PT"
    ),
    paired_group = case_when(
      patient_id %in% paired_patients  ~ "Paired",
      patient_id %in% pt_only_patients ~ "PT only",
      TRUE                             ~ "BM only"
    )
  )

tile_colours <- c(
  "BM (TU-infiltrated)"     = "#E24B4A",
  "BM (non-TU-infiltrated)" = "#F0997B",
  "PT"                      = "#185FA5"
)

group_counts <- tibble(
  paired_group = c("Paired", "PT only", "BM only", "BM controls"),
  label        = c(
    paste0("Paired (n = ",       length(paired_patients),  ")"),
    paste0("PT only (n = ",      length(pt_only_patients), ")"),
    paste0("BM only (n = ",      length(bm_only_patients), ")"),
    paste0("BM controls (n = ",  length(control_patients), ")")
  )
)

group_y_positions <- tibble(
  patient_id   = factor(patient_order, levels = rev(patient_order)),
  paired_group = case_when(
    patient_order %in% paired_patients  ~ "Paired",
    patient_order %in% pt_only_patients ~ "PT only",
    patient_order %in% bm_only_patients ~ "BM only",
    patient_order %in% control_patients ~ "BM controls"
  )
) %>%
  group_by(paired_group) %>%
  summarise(y_mid = median(as.numeric(patient_id)), .groups = "drop") %>%
  left_join(group_counts, by = "paired_group")

p_cohort <- ggplot(cohort_df,
                   aes(x = tissue, y = patient_id, fill = tile_label)) +
  geom_tile(colour = "white", linewidth = 0.8, width = 0.85, height = 0.85) +
  geom_segment(
    data = cohort_df %>%
      filter(patient_id %in% paired_patients) %>%
      distinct(patient_id),
    aes(x = 0.55, xend = 1.45, y = patient_id, yend = patient_id),
...
p_cohort + labs(
  title    = "DE cohort",
  subtitle = paste0(
    "Each row = one patient | dashed lines connect paired BM+PT patient_ids\n",
    ...
    length(control_patients), " controls"
  ),
  x = NULL,
  y = "Patient ID"
)
...
IMC data from BM at diagnosis frequently contains very few tumor cells — in some acquisitions, fewer than 10 TC cells are identified across all ROIs. When TC cluster proportions are derived from such small counts, the resulting composition reflects multinomial sampling noise rather than true biology.

To quantify this uncertainty, I use **non-parametric bootstrap resampling**: for each patient × tissue unit, TC cells are resampled with replacement 1,000 times, and cluster proportions are recomputed each time. The spread of these resampled proportions — measured as the bootstrap standard deviation and 95% confidence interval width — directly quantifies how reliably any given cluster proportion can be estimated from the available cell count.

The key insight is the theoretical 1/√n relationship between cell count and proportion uncertainty: with 10 TC cells, the 95% CI on a cluster at 20% spans roughly 0–55%; with 100 cells it narrows to 12–32%; with 500 cells it reaches 16–24%. patient_ids below ~30 TC cells are therefore treated as unreliable for composition analysis.

## 2.2 Aggregation unit definition

The analysis unit is **patient × tissue** — all ROIs from the same patient in the same tissue are pooled before bootstrapping. This reflects the true biological unit of interest and avoids pseudo-replication at the ROI level.
...
# section 3.4 verdict, so they are derived from results not hardcoded
CLUSTERS_OF_INTEREST <- character(0)
ARTIFACT_CLUSTERS    <- character(0)

# Build TC-only cell table with unit IDs
tc_meta_agg <- seu_meta %>%
  filter(grepl(TC_PATTERN, celltype)) %>%
  select(patient_id, tissue, TC_cluster = celltype) %>%
  mutate(unit_id = paste0(patient_id, " | ", tissue))

# ROIs pooled per unit
# use sample_id (acquisition-level identifier) to count actual ROIs
...
tc_meta_agg %>%
  distinct(unit_id, patient_id, tissue) %>%
  count(tissue) %>%
  knitr::kable(caption = "Number of units per tissue")
```
...
instability_agg <- boot_results_agg %>%
  ...
  separate(unit_id, into = c("patient_id", "tissue"),
           sep = " \\| ", remove = FALSE) %>%
  ...
  mutate(
    ...
    # all units are DE, so is_BM_DE = is_BM
    is_BM_DE = is_BM,
    group    = case_when(
      is_BM  ~ "BM at DE",
      !is_BM ~ "Tumor at DE"
    ) %>% factor(levels = c("BM at DE", "Tumor at DE"))
  )
...
## 2.5 Instability plot — BM units highlighted {.tabset}

The plot below shows the key diagnostic: mean bootstrap SD (y-axis) against pooled TC cell count (x-axis, log scale) for all units. The expected 1/√n decay is visible. BM units are highlighted with bold outlines; dot size reflects the number of ROIs pooled.
...
p_highlighted <- ggplot(instability_agg,
  ...
  scale_x_continuous(trans = "log10", breaks = c(1, 5, 10, 30, 100, 500, 1000),
                     labels = scales::comma) +
  labs(
    title    = "TC cluster call stability — BM units highlighted",
    subtitle = paste0(sum(bm_de_units$flagged), " of ", nrow(bm_de_units),
                      " BM units flagged | one dot = patient x tissue | dot size = ROIs pooled"),
...
p_highlighted
```
**Interpretation: ** Control BM samples (with no TU-infiltration) have tumor cells detected in the IMC data!
More importantly, these are flagged as critical samples, however this is not what we are interested in, in this statistical analysis. 
**Question: ** How do we deal with these? Exclude from analysis? Re-annotate as TU-infiltrated BM?
I would really like to have clinical data on these control samples, as this would be biologically and clinically were important to understand. Have these patients progressed? Was metastasis missed in the initial diagnostic report? Was metastasis detected later and thus the patients were deemed as having disease progression?

### BM bootstrap ribbons

Bootstrap confidence intervals per TC cluster for BM units, sorted by ascending TC cell count. Flagged units are marked with a flag symbol. Wide ribbons with high uncertainty should not be interpreted biologically.
...
plot_df_bm_de <- boot_results_agg %>%
  ...
  mutate(
    unit_label = paste0(ifelse(flagged, "[F] ", ""), unit_id,
...
ggplot(plot_df_bm_de, aes(x = unit_label, colour = reliability, fill = reliability)) +
  ...
  labs(title    = paste0("Bootstrap stability — BM units at ", DE_LABEL),
       subtitle = "Point = observed; thick = IQR; thin = 95% CI | [F] = flagged | sorted low to high pooled TC cells",
...
instability_agg %>%
  ...
  labs(title    = "BM units: mean bootstrap 95% CI width",
...
## 2.6 Low vs. high TU-infiltration in the BM cut-off

Here we want to find a reasonable cut-off to define low vs. high TU-infiltration.
This pertains only to the BM samples (excluding controls).

```{r infiltration-cutoff}
control_patient_ids <- seu_meta %>%
  filter(is_control) %>%
  pull(patient_id) %>%
  unique()

tumor_cells_per_patient <- instability_agg %>%
  filter(is_BM, !patient_id %in% control_patient_ids) %>%
  arrange(n_TC_cells)

quants <- tumor_cells_per_patient %>%
  summarise(
    q10 = quantile(n_TC_cells, 0.10),
    q25 = quantile(n_TC_cells, 0.25),
    med = median(n_TC_cells),
    q75 = quantile(n_TC_cells, 0.75),
    n   = n()
  )
print(quants)

ggplot(tumor_cells_per_patient, aes(x = n_TC_cells)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 40, fill = "grey75", colour = "white", linewidth = 0.3) +
  geom_density(colour = "#185FA5", linewidth = 0.9, adjust = 1.2) +
  geom_rug(aes(colour = reliability), sides = "b", alpha = 0.7, linewidth = 0.5) +
  geom_vline(xintercept = c(10, 30, 100),
             linetype = "dashed", colour = "grey40", linewidth = 0.5) +
  annotate("text", x = c(10, 30, 100), y = Inf,
           label = c("n=10", "n=30", "n=100"),
           hjust = -0.15, vjust = 1.5, size = 3, colour = "grey35") +
  geom_vline(xintercept = quants$med,
             colour = "#E24B4A", linewidth = 0.8, linetype = "solid") +
  annotate("text", x = quants$med, y = Inf,
           label = paste0("median=", round(quants$med)),
           hjust = -0.1, vjust = 2.2, size = 3, colour = "#E24B4A") +
  scale_x_log10(
    breaks = c(1, 3, 10, 30, 100, 300, 1000, 3000),
    labels = scales::comma
  ) +
  scale_colour_manual(values = rel_colours, name = "Reliability tier") +
  labs(
    title    = "TC cell count distribution — BM units (excluding controls)",
    subtitle = paste0("n = ", nrow(tumor_cells_per_patient), " BM units | ",
                      "log10 x-axis | dashed = tier boundaries | red = median | ",
                      "look for natural valley to define low vs high infiltration"),
    x = "Total TC cells per unit (log10)",
    y = "Density"
  ) +
  theme_classic(base_size = 11) +
  theme(
...
table(tumor_cells_per_patient$n_TC_cells > median(tumor_cells_per_patient$n_TC_cells))
```

**Interpretation: ** Log10 x-axis is essential here — TC counts span several orders of magnitude and a linear axis will obscure the low-count region where the cutoff matters most. The natural valley in the density curve between the low-infiltration and high infiltration is our biological cutoff candidate.


```{r}
tumor_cells_per_patient_ctrl <- instability_agg %>%
filter(is_BM, patient_id %in% control_patient_ids) %>%
arrange(n_TC_cells)

quants <- tumor_cells_per_patient_ctrl %>%
summarise(
q10 = quantile(n_TC_cells, 0.10),
q25 = quantile(n_TC_cells, 0.25),
med = median(n_TC_cells),
...
ggplot(tumor_cells_per_patient_ctrl, aes(x = n_TC_cells)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 40, fill = "grey75", colour = "white", linewidth = 0.3) +
  geom_density(colour = "#185FA5", linewidth = 0.9, adjust = 1.2) +
  geom_rug(aes(colour = reliability), sides = "b", alpha = 0.7, linewidth = 0.5) +
  ...
  scale_x_log10(breaks = c(1, 3, 10, 30, 100, 300, 1000, 3000),
                labels = scales::comma) +
  scale_colour_manual(values = rel_colours, name = "Reliability tier") +
  labs(title    = "TC cell count distribution — BM units (only control samples)",
       subtitle = paste0("n = ", nrow(tumor_cells_per_patient), " BM units | ",
                         "log10 x-axis | dashed = tier boundaries | red = median | ",
                         "look for natural valley to define low vs high infiltration"),
       x = "Total TC cells per unit (log10)",
...
ggplot(tumor_cells_per_patient_ctrl, aes(x = patient_id, y = n_TC_cells)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Tumor cells per control BM sample", 
       subtitle = "These samples should not have any tumor cells",
       y = "Number of TU cells", 
       x = "Sample ID") 
...
The three tests below ask complementary questions about whether a TC cluster's apparent presence in low-TC-count patient_ids reflects biology or sampling noise. A cluster flagged by >= 2/3 tests is classified as a probable sampling artifact and excluded from BM composition analyses.
</div>
...
cluster_rank <- boot_results_agg %>%
  ...
  group_by(TC_cluster) %>%
  summarise(overall_mean_SD   = mean(boot_sd),
...
boot_results_agg %>%
  ...
  labs(title    = "Test 1b -- Bootstrap SD distribution per cluster",
       subtitle = "Each dot = one patient x tissue unit",
...
**Question:** Is a cluster detected more often in low-TC-count (flagged) patient_ids than in high-TC-count (reliable) patient_ids? A cluster that disappears as cell count increases is a phantom cluster — it exists only because with few cells, random assignment places 1–2 stray cells there by chance.
...
# NOTE: When very few BM units are flagged (n<30), most TC clusters will have
# pct_detected_flagged = NaN (cluster never seen in the flagged unit).
# NaN causes geom_segment and geom_point to be silently dropped from the plot.
# We replace NaN with 0 for plotting only; label "no flagged detections" explicitly.
artifact_cor_plot <- artifact_cor %>%
  mutate(
    enrich_plot = ifelse(is.nan(enrichment_in_flagged), 0, enrichment_in_flagged),
    label_text  = case_when(
      is.nan(enrichment_in_flagged) ~ paste0("0% (no flagged detections) vs ",
                                              round(pct_detected_reliable, 0), "%"),
      TRUE ~ paste0(round(pct_detected_flagged, 0), "% vs ",
                    round(pct_detected_reliable, 0), "%")
    ),
    TC_cluster = factor(TC_cluster, levels = t2_enrich_order),
    hjust_val  = ifelse(enrich_plot > 0, -0.15, 1.15)
  )

artifact_cor_plot %>%
  ggplot(aes(y = TC_cluster, colour = artifact_label)) +
  geom_vline(xintercept = 0, colour = "grey50", linewidth = 0.5) +
  geom_segment(aes(x = 0, xend = enrich_plot, yend = TC_cluster),
               linewidth = 1.2, alpha = 0.8) +
  geom_point(aes(x = enrich_plot), size = 3.5) +
  geom_text(aes(x = enrich_plot, label = label_text, hjust = hjust_val),
  ...
  labs(title    = "Test 2b -- Detection enrichment in flagged vs reliable units",
       subtitle = paste0("Positive = cluster detected more often in low-n units = artifact signal\n",
                         "NB: Only ", sum(instability_agg$flagged), " flagged unit(s) in this dataset -- ",
                         "NaN values plotted as 0 pp with label"),
...
zero_summary %>%
  ...
  labs(title    = "Test 3 -- 95% CI overlap with zero per cluster",
       subtitle = paste0("Among units passing strict detection threshold (>=", MIN_DETECT_CELLS,
                         ...
                         "NB: 'Flagged units' bar absent for clusters never detected ",
                         "in the single flagged BM unit"),
...
The T2* criterion corrects for a structural blind spot in T1 and T3: very rare clusters never accumulate enough cells to generate bootstrap variance (T1) or pass the strict detection threshold (T3), yet may show the strongest artifact signal in T2. T2* fires when Spearman r < -0.3 AND delta-detect > 30pp.
...
```{r auto-derive-clusters}
# ── Auto-derive ARTIFACT_CLUSTERS and CLUSTERS_OF_INTEREST from verdict ──────
# Artifacts = clusters meeting >=2 criteria; everything else = of interest
ARTIFACT_CLUSTERS <- verdict %>%
  filter(artifact_label == "inconclusive") %>%
  pull(TC_cluster)

CLUSTERS_OF_INTEREST <- setdiff(tc_cols, ARTIFACT_CLUSTERS)

message("Artifact clusters (", length(ARTIFACT_CLUSTERS), "): ",
        paste(ARTIFACT_CLUSTERS, collapse = ", "))
message("Clusters of interest (", length(CLUSTERS_OF_INTEREST), "): ",
        paste(CLUSTERS_OF_INTEREST, collapse = ", "))
```
...
verdict %>%
  ...
  geom_text(aes(x = n_red_flags_revised + 0.05,
                label = paste0(final_verdict,
                               " | T1:", ifelse(t1_flag, "Y", "N"),
                               " T2:", ifelse(t2_flag, "Y", "N"),
                               " T3:", ifelse(t3_flag, "Y",
                                       ifelse(is.na(pct_flagged_overlap), "NA", "N")),
                               " T2*:", ifelse(strong_t2, "Y", "N"),
                               " | r=", round(spearman_r, 2),
                               " | D=", round(enrichment_in_flagged, 0), "pp")),
  ...
  labs(title    = "Combined artifact verdict -- 4 criteria",
       subtitle = "T1: top-half instability | T2: detection enriched in low-n (>=1 cell, >=1%) | T3: CI overlaps zero >50% | T2*: r<-0.3 AND D>30pp",
...
comparison_table %>%
  ...
  scale_colour_manual(values = c("BM -- reliable" = "#E24B4A",
                                  "PT -- reliable" = "#185FA5"),
                      name = NULL) +
  scale_x_continuous(limits = c(0, 110), labels = function(x) paste0(x, "%")) +
  labs(title    = "Detection rate in reliable units: BM vs PT",
       subtitle = "Large BM to PT gap = biologically real in PT but artifactual in BM",
...
enrich_df <- observed_props_agg %>%
  left_join(instability_agg %>% select(unit_id, patient_id, tissue,
                                        n_TC_cells, flagged, reliability),
            by = "unit_id") %>%
  filter(!flagged) %>%
  select(patient_id, tissue, TC_cluster, observed_prop, n_TC_cells, reliability)

both_tissues <- enrich_df %>%
  distinct(patient_id, tissue) %>%
  count(patient_id) %>%
  filter(n == 2) %>%
  pull(patient_id)
...
bm_only_n <- enrich_df %>%
  ...
  anti_join(enrich_df %>% filter(tissue == PT_LABEL) %>% distinct(patient_id),
            by = "patient_id") %>%
  nrow()
...
pt_only_n <- enrich_df %>%
  ...
  anti_join(enrich_df %>% filter(tissue == BM_LABEL) %>% distinct(patient_id),
            by = "patient_id") %>%
  nrow()
...
tibble(
  ...
  knitr::kable(caption = paste0(
    ...
    "Only reliable units (>=30 pooled TC cells) are included. ",
    "Paired patients contribute within-patient BM vs PT contrasts in the LMM."
  ))

run_enrichment_test <- function(cluster, df, paired_patient_ids) {
  bm <- df %>% filter(TC_cluster == cluster, tissue == BM_LABEL)
  pt <- df %>% filter(TC_cluster == cluster, tissue == PT_LABEL)
  bm_paired <- bm %>% filter(patient_id %in% paired_patient_ids) %>% arrange(patient_id)
  pt_paired <- pt %>% filter(patient_id %in% paired_patient_ids) %>% arrange(patient_id)
  unpaired_p <- tryCatch(
...
all_clusters_enrich <- map_dfr(tc_cols, run_enrichment_test,
                                df = enrich_df, paired_patient_ids = both_tissues) %>%
  mutate(
...
all_clusters_enrich %>%
  ...
  labs(title    = "BM vs PT enrichment -- all TC clusters (Unweighted Wilcoxon)",
       subtitle = "Delta = median BM% minus median PT% | BH-adjusted | claim clusters in red",
       x = "Median proportion difference (BM minus PT, percentage points)", y = NULL) +
  theme_classic(base_size = 11) +
  theme(plot.title    = element_text(size = 12, face = "bold", colour = "black"),
        plot.subtitle = element_text(size = 8.5, colour = "grey35"),
        axis.text     = element_text(colour = "black"))
...
- **Model 3 (log count):** Included for transparency only. **Cannot be validly used for BM vs PT comparison** — BM patient_ids are cytospins and PT are tissue sections, so absolute cell counts per ROI are not comparable across tissues.

All models use `log(n_TC_cells)` as a weight, and include a random intercept per patient `(1|patient_id)` to account for the mixed paired/unpaired study design.
</div>
...
reliable_de_units <- instability_agg %>%
  filter(!flagged) %>%
  select(unit_id, patient_id, tissue, n_TC_cells, flagged, reliability)
...
fit_lmm_generic <- function(cluster, df, response_col,
  ...
  if (n_bm < 3 || n_pt < 3 || isTRUE(var(d$y, na.rm = TRUE) == 0))
    return(tibble(TC_cluster = cluster, model = model_name,
                  ...
                  n_patients = n_distinct(d$patient_id), n_paired = NA_integer_,
                  converged = FALSE, note = "insufficient data"))
  n_paired <- d %>% distinct(patient_id, tissue) %>% count(patient_id) %>%
    filter(n == 2) %>% nrow()
  fit <- tryCatch(
    suppressMessages(lmer(y ~ tissue + (1 | patient_id), data = d, weights = w, REML = TRUE,
  ...
  if (is.null(fit))
    return(tibble(TC_cluster = cluster, model = model_name,
                  ...
                  t_value = NA_real_, p_value = NA_real_, mm_BM = NA_real_,
                  mm_PT = NA_real_, n_BM = n_bm, n_PT = n_pt,
                  n_patients = n_distinct(d$patient_id), n_paired = n_paired,
  ...
  tibble(TC_cluster = cluster, model = model_name,
         ...
         n_BM = n_bm, n_PT = n_pt, n_patients = n_distinct(d$patient_id),
  ...
  df %>%
    mutate(
      ...
      sig_label = case_when(p_adj_BH < 0.001 ~ "***", p_adj_BH < 0.01  ~ "**",
                            p_adj_BH < 0.05  ~ "*",   p_adj_BH < 0.10  ~ "ns",
                            TRUE ~ "ns"),
...
all_results <- bind_rows(results_logit, results_clr, results_count) %>%
  ...
  mutate(sd_est     = sd(estimate, na.rm = TRUE),
         ...
         model_label = recode(model,
                              ...
                              log_count        = "log count") %>%
           factor(levels = c("Logit proportion", "CLR proportion", "log count"))) %>%
  ungroup()
...
sig_matrix <- all_results %>%
  mutate(
    cell_label = paste0(sig_label, "\n",
                        ifelse(direction == "BM > PT", "up-BM", "down-PT")),
    fill_val   = case_when(
...
ggplot(sig_matrix, aes(x = model_label, y = TC_cluster, fill = fill_val)) +
  ...
  labs(title    = "Model agreement heatmap -- BM vs PT, all three models",
       subtitle = "Red outline = clusters of interest (auto-derived from verdict)",
       x = NULL, y = NULL) +
  theme_classic(base_size = 11) +
  theme(plot.title    = element_text(size = 12, face = "bold", colour = "black"),
        plot.subtitle = element_text(size = 8.5, colour = "grey35"),
...
if (nrow(claim_logit_data) > 0) {
  ...
  ggplot(claim_logit_mm, aes(x = tissue, y = prop_pct, colour = tissue, fill = tissue)) +
    ...
    geom_line(aes(group = TC_cluster),
...
if (nrow(claim_clr_data) > 0) {
  ...
  sig_clr <- claim_clr_data %>%
    mutate(TC_cluster = factor(TC_cluster, levels = CLUSTERS_OF_INTEREST),
           ...
           label = paste0(sig_label, "\nDelta=", round(mm_BM - mm_PT, 2), " CLR units"))
  ...
  ggplot(claim_clr_mm, aes(x = tissue, y = prop_pct, colour = tissue, fill = tissue)) +
    ...
    labs(title    = "LMM marginal means -- claim clusters (CLR scale, compositionally-corrected)",
         subtitle = "CLR removes sum-to-1 constraint | significant CLR effect = genuine enrichment",
...
verdict %>%
  ...
  mutate(
    recommendation = case_when(
      final_verdict %in% c("artifact (>=3 criteria)",
                            "probable artifact (2 criteria)") ~ "Exclude from BM composition analysis",
      final_verdict == "uncertain (1 criterion)"              ~ "Use in BM with caution -- flag low-n units",
      final_verdict == "biology (0 criteria)"                 ~ "Include -- robust",
      TRUE ~ "Include with caveat"
    )
  ) %>%
  mutate(across(where(is.numeric), ~ round(.x, 3))) %>%
