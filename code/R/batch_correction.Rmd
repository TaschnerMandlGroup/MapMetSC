...
#                assay.type = "exprs", BPPARAM = MulticoreParam())
# 
...
# 
# set.seed(220228)
# spe <- runUMAP(spe, dimred= "fastMNN", name = "UMAP_mnnCorrected", BPPARAM = MulticoreParam()) 
```
...
markers <- c("CD45", "CD3", "CD8a", "CD4", "CD24", "CD14", "CD15", "GD2", "Ki-67", "GD2-IF", "CD56-IF", "CD56")
...
markers are expressed in similar regions of the UMAP. 
...
#https://www.rdocumentation.org/packages/future/versions/1.26.1/topics/future.options
...
p1 <- dittoDimPlot(spe, var = "sample", 
    ...
    theme(legend.text=element_text(size=2))
p2 <- dittoDimPlot(spe, var = "sample", 
                   reduction.use = "UMAP_seurat", size = 0.2, legend.size=2) + 
    scale_color_manual(values = metadata(spe)$color_vectors$sample) +
    ggtitle("Sample ID on UMAP after correction") + 
    theme(legend.text=element_text(size=2))
