...
path <- "/mnt/Multimodal_Imaging_Daria/_Data_Analysis/_tmp_daria/Image_analysis/20230112_DIMR_Ilastik/PT/20230308_sc_data/simple_segmentation"

spe <- readRDS(file.path(path,"data/spe_1px_wo-zeros.rds"))
...
saveRDS(clusters, file.path(path, "data/clusters-int-1px-mean80_post-curation.rds"))
...
column_ha = HeatmapAnnotation(cluster = clusters[rand_cells], 
                              DNA = intensity_mat_lq["193Ir-DNA1_Ir193_mean", rand_cells],
...
saveRDS(mat_red, file.path(path, "data/mat_red_int_post-curation.rds"))
...
for (i in 1: nrow(mat_tmp)){
  ...
  ggsave(filename=file.path(path, paste0("data/UMAP_single_marker_post-curation", marker , ".tiff")), width = 10, height = 5, units = "in", device='tiff', dpi=500)
} 
```
...
ggsave(filename=file.path(out_path, "UMAP_single_marker_int_v2.tiff"), width = 7, height = 30, units = "in", device='tiff', dpi=500)
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(path, paste("/data/UMAP_1px-mean_single_sample_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
...
```{r barplot-cluster-per-sample, fig.width=20}
 # library
library(ggplot2)
library(tidyverse)
...
plotCells(masks,
          ...
          display = "single",
          save_plot = list(filename = file.path(path, "data/20220222_16-4503_BM_ROI_001_allclusters.png")),
          legend = NULL)
...
plotCells(masks,
          ...
          display="single",
          save_plot = list(filename = file.path(path, paste0("data/20220222_16-4503_BM_ROI_001_cluster_", c, ".png"))),
          legend=NULL)
