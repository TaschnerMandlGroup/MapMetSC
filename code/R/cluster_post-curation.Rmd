...
saveRDS(clusters, file.path(path, "data/clusters-int-1px-mean_postcuration_2.rds"))

```

Next we will look at the single cell heatmap ordered by the identified phenograph clusters. 

```{r single-cell-heatmap-clusters, message=FALSE, fig.height=7}
library(ComplexHeatmap)
library(viridis)
library(circlize)
library(RColorBrewer)
...
```{r ingle-cell-heatmap-clusters-mean, message=FALSE, fig.height=7}
library(scuttle)
...
```{r barplot-cluster-per-sample, fig.width=8}
 # library
library(ggplot2)
library(tidyverse)
