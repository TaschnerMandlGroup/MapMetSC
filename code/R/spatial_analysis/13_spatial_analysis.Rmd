...
for_plot <- table(as.character(spe$cn_celltypes_2), spe$celltype_2)
...
for_plot <- prop.table(table(as.character(spe$cn_celltypes_2), spe$celltype_2), margin = 2)
...
```{r, fig.width=15, fig.height=6}
for_plot <- prop.table(table(as.character(spe$cn_celltypes_2), spe$celltype_2), margin = 1)
...
plot_list <- list()
celltypes <- unique(spe[, spe$metacluster=="T cell" | spe$metacluster=="B cell" | spe$metacluster=="granulocyte" | spe$metacluster=="MO/DC/NK"]$celltype_3)
...
```{r}
plotSpatial(spe[,spe$sample_id == "20220926_20220809_16-1934_TU_003"], 
...
plotSpatial(spe_red[,spe_red$sample_id == "20220926_20220809_16-1934_TU_003"], 
...
```{r, fig.width=10, fig.height=10}
library(tidyr)
df <- as.data.frame(colData(spe)[(spe$metacluster =="T cell" | spe$metacluster =="MO/DC/NK" | spe$metacluster =="B cell" | spe$metacluster =="granulocyte"), c("sample", "metacluster", "border")])

colnames(df) <- c("sample", "celltype", "border")
...
df_normalized <- df_summary %>%
  group_by(celltype, border) %>%
  summarise(normalized_prop = mean(proportion, na.rm = TRUE), std=sd(proportion, na.rm = TRUE)) %>%
  ungroup()

df_normalized$celltype <- factor(df_normalized$celltype)
levels

ggplot(df_normalized, aes(fill=celltype, y=normalized_prop, x=border)) + 
...
spe_red <- spe[,spe$metacluster!="other"]
...
saveRDS(out, file.path(path,"results/spatial_analysis/out_celltype2.rds"))
```
...
saveRDS(spe, file.path(path,"NEWEST_RESULTS/results/spatial_analysis/spe_CNs_border2.rds"))
```
