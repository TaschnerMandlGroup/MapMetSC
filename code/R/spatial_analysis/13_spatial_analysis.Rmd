...
```{r, fig.width=15, fig.height=6}
for_plot <- prop.table(table(as.character(spe$cn_celltypes), spe$metacluster), margin = 1)
...
```{r patchDetection-2, fig.width=20, fig.height=10, message=FALSE}
library(tidyverse)
...
Tcell_freq_per_patch <- colData(spe) %>% as_tibble() %>%
    group_by(patch_id, sample) %>%
    summarize(Tcell_count = sum(metacluster == "T cell"),
              patch_size = n(),
              Tcell_freq = Tcell_count / patch_size) %>%
...
mean_tcell_freq_per_sample <- Tcell_freq_per_patch %>%
  group_by(sample) %>%
  summarise(mean_Tcell_freq = mean(Tcell_freq, na.rm = TRUE))%>%
  arrange(desc(mean_Tcell_freq))
...
combined_data <- mean_tcell_freq_per_sample %>%
  left_join(progression_per_sample, by = "sample")

stats_df <- compare_means(mean_Tcell_freq ~ condition,  
                          method = "wilcox.test",  
                          data = combined_data) 
...
p <- ggboxplot(combined_data, x = "condition", y = "mean_Tcell_freq",
                color = "condition", #palette =c("slategray1", "slategray"),
                add.params = list(size = 3),
                add = "jitter")+ 
                stat_pvalue_manual(stats_df, label = "p_long", y.position = max(combined_data$mean_Tcell_freq), step.increase = 0.2,) +
...
```{r celltype-distance, fig.height=15,fig.width=10, message=FALSE}
library(ggridges)
