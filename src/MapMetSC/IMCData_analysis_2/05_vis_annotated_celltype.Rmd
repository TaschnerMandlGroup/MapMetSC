...
spe <- readRDS(file.path(path,"results/spe_clustered.rds"))
...
for(i in 1: length(unique(celltype))){
  ...
  png(file=file.path(path, paste0("results/heatmaps/annotated/cluster_low_res/", c , ".tiff")),width=18,height=8,units="in", res=1200)
...
mean_t_wo_sticky <- mean_t[,levels(celltype)!=sticky_cluster]
mean_t_wo_sticky <- t(apply(mean_t_wo_sticky, 1, RESCALE, to=0:1))
clusters_wo_sticky <- droplevels(clust[clust!=sticky_cluster])

area_mean_wo_sticky <- area_mean[levels(celltype)!=sticky_cluster]
solidity_mean_wo_sticky <- solidity_mean[levels(celltype)!=sticky_cluster]
aspect_ratio_mean_wo_sticky <- aspect_ratio_mean[levels(celltype)!=sticky_cluster]
clPARP_mean_wo_sticky <- clPARP_mean[levels(celltype)!=sticky_cluster]
...
col_celltype <- metadata(spe)$color_vector$col_celltype
col_clusters_wo_sticky <- col_celltype[names(col_celltype)!=sticky_cluster]

column_ha = HeatmapAnnotation(celltype = droplevels(mean[levels(celltype)!=sticky_cluster,"Group.1"]),
                              ...
                              col=list(celltype=col_clusters_wo_sticky,
