...
#clipped_area <- c(clipper(morph_mat[1,tissue=="PT"], 0, area_upper))
...
library(batchelor)
set.seed(220228)
out <- fastMNN(spe, batch = spe$tissue,
               auto.merge = FALSE,
...
saveRDS(out, file.path(path, "results/fastMNN.rds"))
```

We can also use UMAP for dimensionality redcution before clustering.
...
mat_red <- calculateUMAP(mat, ncomponents=3, n_neighbors=30, min_dist= 0, BPPARAM = MulticoreParam())
rownames(mat_red) <- colnames(spe)

saveRDS(mat_red, file.path(path, "results/mat_red_3components.rds"))

# mat_red <- readRDS(file.path(path,"results/mat_red_final.rds"))
# rownames(mat_red) <- colnames(spe)

#Apply changes to SPE object
reducedDim(spe, "UMAP_3") <- mat_red
```
...
# mat_integrated <- reducedDim(spe, "fastMNN")
# non_zeros <- as.logical(rowSums(mat_integrated)!=0)
# mat_integrated <- mat_integrated[non_zeros,] 
# #mat <- t(mat)
# 
# out <- Rphenograph(mat_integrated, k = 45)
# 
...
saveRDS(clusters, file.path(path, "results/clusters_UMAP-3-dim.rds"))

# clusters <- readRDS(file.path(path, "results/clusters_final.rds"))

#Apply changes to SPE object
spe$pg_clusters <- clusters
```

We will now cluster the data using HDBscan. 

```{r HDBScan-clustering}
library("dbscan")

cl <- hdbscan(mat_red, minPts=100)
clusters <- cl$cluster

spe$hdbscan_clusters <- clusters

saveRDS(clusters, file.path(path, "results/hdbscan_clusters_UMAP-5-dim.rds"))
```
```{r GMM-clustering}
library("ClusterR")
...
for(i in 1: length(unique(clusters))){
  ...
  png(file=file.path(path, paste0("results/dim_reduction/UMAP_3comp_phenograph/heatmaps/", c , ".tiff")),width=18,height=8,units="in", res=1200)
...
saveRDS(mat_red, file.path(path, "results/mat_red_k30.rds"))
...
for (i in 1: nrow(mat_tmp)){
  ...
  ggsave(filename=file.path(path, paste0("results/dim_reduction/UMAP_phenograph/markers_decreasing/", marker , ".tiff")), width = 10, height = 5) #units = "in", device='tiff', dpi=500)
} 
```
...
save_tiff <- function(sublist, j){
  ...
  ggsave(filename=file.path(path, paste("/results/UMAP/sample/sample_", j, ".tiff", sep="")), width = 7, height = 20, units = "in", device='tiff', dpi=500)
}
...
```{r barplot-tissues-per-cluster, fig.width=10}
...
saveRDS(spe, file.path(path, "results/spe_processed.rds"))
```
