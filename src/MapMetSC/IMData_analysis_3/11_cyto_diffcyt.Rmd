...
keep_cols <- spe$tissue=="BM" & spe$control=="no" & spe$metacluster=="tumor" &(spe$timepoint=="DE" | spe$timepoint=="RE1") & spe$study_id%in%paired_patients
...
pbMDS(sce, by="sample_id", color_by="condition", label_by=NULL)
```
...
```{r diff-freqs-plot-props-barplot, fig.cap = "Relative abundance of the 8 PBMC populations in each sample (x-axis), in the PBMC dataset, represented with a barplot. The 8 cell populations are a result of manual merging of the 20 FlowSOM metaclusters", fig.height=8, fig.width=18}
#
...
# 
# NoProg.RE2 <- progression == "NoProg" & condition=="RE2"
# Prog.RE2 <- progression == "Prog" & condition=="RE2"
# 
# design <- cbind(design, nMNA.RE2, MNA.RE2, unknown.RE2)
# design <- cbind(design, NoProg.RE2, Prog.RE2)
```
